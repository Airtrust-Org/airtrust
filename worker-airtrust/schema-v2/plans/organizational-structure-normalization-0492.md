...
- CTM is part of Manutenção: the tenant-local sector identified as `CTM` merges into canonical `Manutenção`; numeric IDs are not migration authority.
- Qualidade is QSMS: the tenant-local sector identified as `Qualidade` merges into canonical `QSMS`; numeric IDs are not migration authority.
...
- sector references in `funcionarios`, `qualificacoes_tipos_setores`, `lms_cursos_setores`, `setores_gestores`, and `treinamento_requisitos` are reconciled by tenant-local code/name identity before source sectors are soft-deleted;
