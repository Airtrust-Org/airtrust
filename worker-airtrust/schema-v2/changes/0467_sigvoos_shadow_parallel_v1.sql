-- Production Schema V2 change for 0467 SIGVOOS shadow/parallel V1.
-- Additive-only shadow/audit structures. These tables are not an FRMS
-- operational authority and do not change the canonical SIGVOOS -> FRMS path.
--
-- source_reference:
--   worker-airtrust/migrations/0467_sigvoos_shadow_parallel_v1.sql
-- operational_decision:
--   Reproduce the reviewed additive shadow schema in production only through
--   Schema V2; no backfill, sync, FRMS recalculation, or operational cutover.
-- dry_run_required:
...
--   leaves additive shadow tables inert; destructive cleanup is separate.
