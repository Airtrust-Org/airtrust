    ...
    try {
      ...
      for (const orfao of orfaos) {
        try {
          ...
          const cpfIndex = parts.findIndex((part) => /^\d{11}$/.test(part));
          const codigo = cpfIndex > 1 ? parts[cpfIndex - 1] : parts[2];
          const dataToken = parts.find((part) => /^\d{8}$/.test(part));
          const dataReferencia = dataToken
            ? `${dataToken.slice(0, 4)}-${dataToken.slice(4, 6)}-${dataToken.slice(6, 8)}`
            : String(orfao.documento_data || '').split('T')[0];

          console.log(
            `🔍 [RECUPERAR] Documento ${orfao.documento_id} (${orfao.nome_arquivo}) → buscando qual ${codigo} em ${dataReferencia}`,
          );

          // A data persistida no nome/arquivo é a evidência mais forte. O id mais recente
          // só desempata candidatos igualmente próximos.
          const { results: candidatos } = await db
            ...
            WHERE qh.funcionario_id = ?
              AND (qh.qualificacao_codigo = ? OR qh.codigo = ?)
              AND f.empresa_id = ?
              AND qh.certificado_arquivo_id IS NULL
              AND qh.deleted_at IS NULL
            ORDER BY
              CASE WHEN date(qh.data_conclusao) = date(?) THEN 0 ELSE 1 END,
              ABS(julianday(qh.data_conclusao) - julianday(?)) ASC,
              qh.id DESC
            LIMIT 1
            `,
            )
            .bind(orfao.funcionario_id, codigo, codigo, empresaId, dataReferencia, dataReferencia)
  ...
  async (c) => {
    ...
    try {
      const hasOrigemTipo = await db
        .prepare(
          `SELECT COUNT(*) as n FROM pragma_table_info('qualificacoes_historico') WHERE name='origem_tipo'`,
        )
        .first<{ n: number }>();
