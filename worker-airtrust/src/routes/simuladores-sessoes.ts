...
// ─────────────────────────────────────────────────────────────────────────────
async function criarQualificacoesPlanejadas(
  db: D1Database,
  params: {
    sessaoId: number;
    modeloId: number;
  ...
  // Buscar modelo + qualificacao_tipo em um único query
  ...
  for (const part of params.participantes) {
    // Deduplicar: não criar se já existe qualquer registro (PLANEJADA ou não) para esta sessão+participante
...
app.post('/sessoes', async (c) => {
  try {
    ...
    const modeloIdParaQual = Number(templateIdFinal || modelo_sessao_id || 0) || null;
    if (modeloIdParaQual) {
      try {
