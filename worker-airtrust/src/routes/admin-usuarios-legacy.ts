...
async function sendInviteEmail(
  ...
  try {
    ...
    logger.warn(`Falha ao enviar e-mail de convite para ${payload.email}:`, {
...
adminUsuariosRoutes.post('/', async (c) => {
  ...
  if (isManager) {
    try {
      setorIdsValidados = await assertSetoresValidosParaEmpresa(
        db,
        targetEmpresaId,
        body?.setor_ids,
      );
...
adminUsuariosRoutes.put('/:id', async (c) => {
  ...
  const tornandoSeGestor =
    Boolean(body?.perfil) && isManagerPerfil(targetPerfil) && !isManagerPerfil(existente.perfil);
