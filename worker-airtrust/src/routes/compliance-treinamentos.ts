...
type CompliancePerson = Awaited<ReturnType<typeof buildSnapshot>>['people'][number];
...
async function loadComplianceNotificationStats(db: D1Database, empresaId: number) {
  ...
  try {
    const rows = await db
      .prepare(
        `SELECT tipo, corpo, status, COALESCE(enviado_em, created_at) AS event_at
      ...
      .bind(empresaId)
      .all<{ tipo: string | null; corpo: string | null; status: string | null; event_at: string | null }>();
    for (const row of rows.results || []) {
      let body: Record<string, unknown> | null = null;
      try {
        body = row.corpo ? (JSON.parse(row.corpo) as Record<string, unknown>) : null;
      } catch {
        body = null;
      }
      const funcionarioId = Number(body?.funcionario_id || 0);
      const tipoId = Number(body?.qualificacao_tipo_id || 0);
      if (!funcionarioId || !tipoId) continue;
      const key = `${funcionarioId}:${tipoId}`;
      const current = map.get(key) || { count: 0, last_at: null, last_channel: null, last_status: null };
      if (row.status === 'enviada') current.count += 1;
      if (!current.last_at) {
        current.last_at = row.event_at || null;
        current.last_channel = row.tipo || null;
        current.last_status = row.status || null;
      }
      map.set(key, current);
    }
  } catch {
