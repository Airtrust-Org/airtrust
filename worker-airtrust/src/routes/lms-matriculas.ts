...
/*                                                                             */
/* Read model puramente INFORMATIVO. Nunca altera lesson_status, score, status  */
/* de matrícula, qualificação ou certificado — a autoridade canônica de         */
/* conclusão continua sendo `completion_diagnostic`.                            */
/* -------------------------------------------------------------------------- */
 ...
 */
async function resolveMatriculaForDiagnostics(
  c: Context,
  db: D1Database,
  matriculaId: number,
...
app.put('/:id/completion-diagnostics', async (c) => {
  ...
  const empresaId = getEmpresaIdSafe(c);
  const matriculaId = Number(c.req.param('id'));
  const matricula = await resolveMatriculaForDiagnostics(c, db, matriculaId);
  ...
  const raw = (body as { diagnostics?: unknown } | null)?.diagnostics;
  const snapshot = parseCompletionDiagnosticsSnapshot(raw);
