...
function run(db: string, sql: string) { return spawnSync('sqlite3', ['-bail', db], { input: sql, encoding: 'utf8' }); }
function query(db: string, sql: string) { return spawnSync('sqlite3', ['-json', db, sql], { encoding: 'utf8' }); }
const schema = `
PRAGMA foreign_keys=ON;
CREATE TABLE empresas(id INTEGER PRIMARY KEY);
CREATE TABLE modelos_sessao(id INTEGER PRIMARY KEY, codigo TEXT NOT NULL UNIQUE, empresa_id INTEGER NOT NULL, deleted_at TEXT);
...
CREATE TABLE fichas_sessao(id INTEGER PRIMARY KEY, modelo_id INTEGER REFERENCES modelos_sessao(id));
CREATE TABLE simulador_agendamentos(id INTEGER PRIMARY KEY, modelo_sessao_id INTEGER REFERENCES modelos_sessao(id));
`;
...
INSERT INTO modelos_sessao VALUES(10,'OLD-A',7,NULL),(20,'OTHER',8,NULL),(30,'DELETED',7,'2020-01-01');
...
INSERT INTO fichas_sessao VALUES(1,10); INSERT INTO simulador_agendamentos VALUES(1,10);`);
      ...
      expect(run(db, "UPDATE simuladores_matriz_imports SET status='APPLIED' WHERE uuid='import-1'").status).toBe(0);
      expect(query(db, 'SELECT modelo_id FROM fichas_sessao WHERE id=1').stdout).toContain('10');
      expect(query(db, 'SELECT modelo_sessao_id FROM simulador_agendamentos WHERE id=1').stdout).toContain('10');
...
INSERT INTO empresas VALUES(7); INSERT INTO modelos_sessao VALUES(10,'OLD-A',7,NULL); INSERT INTO manobras VALUES(100,7),(101,7);
INSERT INTO modelos_sessao_manobras VALUES(1,10,100,1,1,NULL,NULL,NULL,NULL,NULL,NULL,'AB'),(2,10,101,1,1,NULL,NULL,NULL,NULL,NULL,NULL,'AB');`).status).toBe(0);
  ...
  it('fails preflight for a legacy manoeuvre linked across tenants', () => {
    ...
    try {
      expect(run(db, `${schema}
INSERT INTO empresas VALUES(7),(8); INSERT INTO modelos_sessao VALUES(10,'OLD-A',7,NULL); INSERT INTO manobras VALUES(200,8);
