...
import { gerarQualificacaoDaFicha, isTipoCurricularSemestral } from '../../routes/simuladores-fichas-helpers';

function createDb(tipoCurricularCodigo: string | null, checks: unknown[]) {
  const ficha = {
    empresa_id: 17,
    aluno_empresa_id: 17,
    ...
    tipo_curricular_nome: tipoCurricularCodigo === 'SEM' ? 'Semestral' : (tipoCurricularCodigo ? 'Periódico' : null),
  };
  ...
  return {
    prepare: vi.fn((query: string) => ({
      bind: vi.fn(() => ({
        ...
        all: vi.fn(async () => ({ results: query.includes('FROM sessoes_checks') ? checks : [] })),
      })),
    })),
...
describe('G1-SEM curricular gate', () => {
  beforeEach(() => {
    ...
    mocks.g1Sem.mockReset().mockResolvedValue({ id: 991, dataVencimento: '2027-01-21' });
  ...
  it('generates the periodic principal and approved six-month FAP without G1-SEM for PER', async () => {
    const result = await gerarQualificacaoDaFicha(createDb('PER', [{
  ...
  it('generates G1-SEM once for SEM with an approved six-month FAP', async () => {
    const result = await gerarQualificacaoDaFicha(createDb('SEM', [{
      qualificacao_tipo_id: 13,
      qt_codigo: 'FAP06',
      qt_nome: 'FAP seis meses',
      qt_validade: 6,
  ...
  it('fails closed and emits a sanitized warning when the curricular type is absent', async () => {
    ...
    await gerarQualificacaoDaFicha(createDb(null, [{
      qualificacao_tipo_id: 13,
      qt_codigo: 'FAP06',
      qt_nome: 'FAP seis meses',
      qt_validade: 6,
