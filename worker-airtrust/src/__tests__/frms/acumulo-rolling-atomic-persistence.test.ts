...
describe('persistirAcumuloRolling atomic persistence', () => {
  ...
  it('falls back atomically to the pre-0498 rolling shape during code-first rollout', async () => {
    const prepared: Array<{ sql: string; args: unknown[] }> = [];
    const prepare = vi.fn((sql: string) => ({
      bind: (...args: unknown[]) => {
        const statement = { sql, args };
    ...
    const batch = vi
      .fn()
      .mockRejectedValueOnce(new Error('table frms_acumulo_rolling has no column named hv_ano_calendario_min'))
      .mockResolvedValueOnce([{ success: true }, { success: true }]);
    const db = { prepare, batch } as unknown as D1Database;

    await persistirAcumuloRolling(db, 42, '2026-09-10', {
      hv_7_dias_min: 120, hv_28_dias_min: 360, hv_365_dias_min: 1200,
      hv_ano_calendario_min: 900, hv_mes_calendario_min: 240, hv_dia_min: 60,
      pct_limite_7d: 10, pct_limite_28d: 20, pct_limite_mes_calendario: 30,
      pct_limite_365d: 40, pct_limite_ano_calendario: 35, pct_limite_dia: 50,
      repouso_anterior_min: 720, repouso_suficiente: 1,
    });

    expect(batch).toHaveBeenCalledTimes(2);
    const legacyBatch = batch.mock.calls[1]?.[0] as Array<{ sql: string; args: unknown[] }>;
    expect(legacyBatch).toHaveLength(2);
    expect(legacyBatch[0]?.sql).toContain('UPDATE frms_acumulo_rolling SET deleted_at');
    expect(legacyBatch[1]?.sql).not.toContain('hv_ano_calendario_min');
    expect(legacyBatch[1]?.sql).not.toContain('pct_limite_ano_calendario');
  });
