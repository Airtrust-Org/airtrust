...
describe('FRMS operational parameters are DB-backed', () => {
  it('loads a complete configured parameter set from D1', async () => {
    const rows = Object.keys(LIMITES_DEFAULT).map((nome, index) => ({ nome, valor_numerico: index + 0.25 }));
    const resolved = await carregarLimites(dbWith(rows));
    for (const row of rows) expect(resolved[row.nome as keyof typeof resolved]).toBe(row.valor_numerico);
  });
  ...
  it('fails closed when a required operational parameter is missing instead of using a hardcoded value', async () => {
    ...
    await expect(carregarLimites(dbWith(rows))).rejects.toThrow('FRMS_OPERATIONAL_PARAMETER_MISSING');
  });

  it('fails closed when D1 cannot load parameters instead of silently restoring code defaults', async () => {
    await expect(carregarLimites(dbWith([], new Error('D1 unavailable')))).rejects.toThrow('D1 unavailable');
  });
});
