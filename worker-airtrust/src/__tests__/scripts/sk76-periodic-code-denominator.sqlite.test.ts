...
function buildDatabase() {
  const dir = mkdtempSync(join(tmpdir(), 'sk76-periodic-code-'));
  const dbPath = join(dir, 'test.sqlite');

  const schema = execSql(
    dbPath,
    `
  ...
  for (const [index, [legacy]] of SK76_PERIODIC_CODE_RENAMES.entries()) {
    const id = index + 1;
    const seed = execSql(
      dbPath,
      `
        INSERT INTO modelos_sessao(id,codigo,empresa_id)
        VALUES (${id},${sqlText(`${legacy}@M2026.07-V1`)},6);
        INSERT INTO modelos_sessao_versionamento(
          ...
          ${id},6,${sqlText(legacy)},1,'M2026.07',1,NULL,CURRENT_TIMESTAMP,NULL,
        ...
        ) VALUES (${id},6,${sqlText(legacy)},'PERIODICO',NULL,NULL,NULL,NULL);
      `,
    );
    expect(seed.code, seed.stderr).toBe(0);
  }

  const otherTenant = execSql(
    dbPath,
    `
      INSERT INTO modelos_sessao(id,codigo,empresa_id)
      VALUES (100,'S76-P-01/04-C1@TENANT7-V1',7);
      INSERT INTO modelos_sessao_versionamento(
        modelo_id,empresa_id,codigo_canonico,versao_numero,versao_matriz,is_current,
        modelo_anterior_id,efetivo_em,efetivo_ate,created_at,updated_at
      ) VALUES (
        100,7,'S76-P-01/04-C1',1,'M2026.07',1,NULL,CURRENT_TIMESTAMP,NULL,
        CURRENT_TIMESTAMP,CURRENT_TIMESTAMP
      );
      INSERT INTO simuladores_guias_instrutor(
        id,empresa_id,codigo,programa,deleted_at
      ) VALUES (100,7,'S76-P-01/04-C1','PERIODICO',NULL);
    `,
  );
  expect(otherTenant.code, otherTenant.stderr).toBe(0);
...
describe('S-76 periódico — denominador canônico /03', () => {
  ...
  it('expõe /03 no contrato efetivo mesmo com o snapshot-fonte histórico ainda em /04', () => {
    ...
    const codes = contract.sessions.map(
      (session: { codigo_canonico: string }) => session.codigo_canonico,
    );
  ...
  it('migration 0459 preserva IDs e vínculos e corrige somente o tenant 6', () => {
    const { dbPath, cleanup } = buildDatabase();
    try {
      const beforeLinks = querySql<{ c: number }>(
        dbPath,
        'SELECT COUNT(*) AS c FROM modelos_sessao_manobras;',
      )[0];

      const migrated = execSql(dbPath, migration);
      expect(migrated.code, migrated.stderr).toBe(0);

      const currentCodes = querySql<{ codigo_canonico: string }>(
        dbPath,
      ...
      const physical = querySql<{ id: number; codigo: string }>(
        dbPath,
        'SELECT id,codigo FROM modelos_sessao WHERE empresa_id=6 ORDER BY id;',
      );
      expect(physical.map((row) => row.id)).toEqual([1, 2, 3, 4, 5, 6]);
      expect(physical.every((row) => row.codigo.includes('/03'))).toBe(true);

      const guides = querySql<{ codigo: string }>(
        dbPath,
        'SELECT codigo FROM simuladores_guias_instrutor WHERE empresa_id=6 ORDER BY id;',
      );
      expect(guides.every((row) => row.codigo.includes('/03'))).toBe(true);

      const afterLinks = querySql<{ c: number }>(
        dbPath,
        'SELECT COUNT(*) AS c FROM modelos_sessao_manobras;',
      )[0];
      expect(afterLinks.c).toBe(beforeLinks.c);

      const tenant7 = querySql<{ codigo_canonico: string }>(
        dbPath,
      ...
      const immutable = execSql(
        dbPath,
        "UPDATE modelos_sessao_versionamento SET codigo_canonico='ALTERADO' WHERE empresa_id=6 AND modelo_id=1;",
      );
      expect(immutable.code).not.toBe(0);
      expect(immutable.stderr).toMatch(/identidade de versão é imutável/);
    } finally {
      cleanup();
    }
  });
