...
// The 23 real canonical codes confirmed absent tenant-wide (and cross-tenant)
// during the 2026-07-22 reconciliation of the AW139/S-76 matrix import. Only
// the technical code and its verified classification are versioned here, per
// the sanitization rule: no names, categories, or operational descriptions.
const REAL_TRUE_MISSING_CODES = [
  'ABN-ANO-69',
  'ABN-HNG-69',
  'ABN-MRC-30B',
  'ABN-NLO-69',
...
describe('matriz-manobra-resolution: classification', () => {
  ...
  it('classifies the real 23 codes as TRUE_MISSING (verified absent tenant-wide and cross-tenant)', () => {
    for (const codigo of REAL_TRUE_MISSING_CODES) {
      const result = classifyManoeuvreCode({
...
describe('matriz-manobra-resolution: building + validating the 301-entry block', () => {
  it('builds and validates exactly 301 resolutions: 278 EXACT_UNIQUE + the real 23 TRUE_MISSING', () => {
    const items = fullFixtureItems(REAL_TRUE_MISSING_CODES);
  ...
  it('rejects an incomplete create_payload for a TRUE_MISSING/COLLISION/CROSS_TENANT_ONLY entry', () => {
    ...
    const entries = buildManoeuvreResolutionEntries({ empresaId: 6, items, tenantManobras });
    const mutated = entries.map((e) =>
      e.resolution_type === 'TRUE_MISSING' && e.codigo_canonico === REAL_TRUE_MISSING_CODES[0]
  ...
  it('accepts a human-reviewed FORMAL_ALIAS/LEGACY_EQUIVALENT override with evidence', () => {
    const items = fullFixtureItems(REAL_TRUE_MISSING_CODES.slice(0, 1));
    const tenantManobras = tenantManobrasFor(items, 6);
    const legacyId = 555;
    const overrides = {
      [REAL_TRUE_MISSING_CODES[0]]: {
    ...
    const entry = entries.find((e) => e.codigo_canonico === REAL_TRUE_MISSING_CODES[0]);
