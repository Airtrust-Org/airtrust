      ...
      bind: (...bindArgs: unknown[]) => {
        ...
        if (
          ...
          return {
            all: async () => ({
              results: cpfs
                .filter((cpf) => createdCpfs.has(cpf))
                .map((cpf) => ({ cpf })),
            }),
          };
        }
        ...
        if (
          normalizedQuery.includes('SELECT UPPER(codigo) as codigo FROM qualificacoes_tipos WHERE UPPER(codigo) IN') &&
          normalizedQuery.includes('empresa_id = ?')
        ) {
        ...
        if (normalizedQuery.includes('INSERT INTO funcionarios') && normalizedQuery.includes('empresa_id')) {
        ...
        if (normalizedQuery.includes('INSERT INTO qualificacoes_tipos') && normalizedQuery.includes('empresa_id')) {
          for (let i = 0; i < bindArgs.length; i += 6) {
        ...
        if (
          normalizedQuery.includes('SELECT id, UPPER(codigo) as codigo, validade, vencimento_fim_mes') &&
          normalizedQuery.includes('empresa_id = ?')
        ) {
          const codigos = bindArgs.slice(0, -1).map((v) => String(v).toUpperCase());
          return {
            all: async () => ({
              results: codigos
                .filter((codigo) => createdCodigos.has(codigo))
                .map((codigo, idx) => ({ id: idx + 1, codigo, validade: null, vencimento_fim_mes: 0 })),
            }),
          };
        }

        if (normalizedQuery.includes('INSERT INTO qualificacoes_historico') && normalizedQuery.includes('empresa_id')) {
          return { run: async () => ({ success: true }) };
        }
...
describe('QualificacaoHistoricoImportacaoService tenant scope', () => {
  it('usa empresa_id em auto-create e insert do histórico', async () => {
    ...
    const hasFuncionarioInsertTenant = calls.some(
      (call) =>
        call.query.includes('INSERT INTO funcionarios') &&
    ...
    expect(hasFuncionarioInsertTenant).toBe(true);
