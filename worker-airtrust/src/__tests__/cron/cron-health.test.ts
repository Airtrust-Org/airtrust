...
describe('cron resilience health', () => {
  ...
  it('classifica falhas, lease, backlog, atraso, cursor parado e run presa', async () => {
    const all = vi.fn().mockResolvedValue({
      results: [
        {
          job_name: 'ead-renewal',
          scope_key: 'empresa:6',
          last_started_at: '2026-08-02 06:00:00',
          updated_at: '2026-08-02 06:00:00',
          ...
          exhausted_items: 2,
          latest_outcome: 'RUNNING',
          latest_started_at: '2026-08-02 07:00:00',
          latest_cursor: '10',
          previous_cursor: '10',
          latest_processed: 0,
    ...
    expect(bind).toHaveBeenCalledWith(CRON_HEALTH_RETRY_THRESHOLD, CRON_HEALTH_STATE_LIMIT);
    expect(snapshot).toMatchObject({
      ...
      delayedScopes: 1,
      staleRunningScopes: 1,
      tenantErrorScopes: 1,
      pendingItems: CRON_HEALTH_BACKLOG_THRESHOLD,
      exhaustedItems: 2,
    });
    expect(snapshot.affectedScopes[0]).toMatchObject({
      job_name: 'ead-renewal',
      scope_key: 'empresa:6',
      error_code: 'EAD_RENEWAL_REPAIR_FAILED',
    });
    expect(snapshot.affectedScopes[0].signals).toEqual(
