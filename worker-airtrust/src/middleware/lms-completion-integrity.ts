...
function resolveProgressPct(row: EnrollmentEvidenceRow, incoming: JsonRecord): number {
  ...
  return Math.max(existing, incomingLocation ?? 0, storedLocation ?? 0, incomingScore ?? 0, storedScore ?? 0);
}
...
function isInteractive(row: EnrollmentEvidenceRow): boolean {
  const type = String(row.tipo_conteudo ?? 'scorm').trim().toLowerCase();
...
async function guardXapiStatement(
  ...
  if (ownershipError) return ownershipError;
  const assetSessionValid = await hasValidAssetSession(c, row);
  const score = result.score && typeof result.score === 'object' ? (result.score as JsonRecord) : {};
    ...
    lesson_status: verb.endsWith('/failed')
      ...
      : verb.endsWith('/passed')
        ...
        : verb.endsWith('/completed')
          ? 'completed'
          : null,
    completion_status: result.completion === true ? 'completed' : result.completion === false ? 'incomplete' : null,
...
async function guardAdministrativeStatus(
  ...
  const incoming = await readJsonClone(c);
  const requestedStatus = String(incoming.status ?? '').trim().toUpperCase();
  const row = await readEnrollmentEvidence(c.env.DB, empresaId, matriculaId);
  if (!row) return errorResponse(c, 404, 'LMS_ENROLLMENT_NOT_FOUND', 'Matrícula não encontrada.');
  ...
  if (String(row.status).toUpperCase() === 'CONCLUIDO' && requestedStatus !== 'CONCLUIDO') {
    return errorResponse(
      ...
      { matricula_id: matriculaId, reversal_endpoint: `/api/lms/matriculas/${matriculaId}/reverter` },
    );
  }
  if (requestedStatus !== 'CONCLUIDO') return null;

  const administrativeAuthorized = hasRole(c, 'admin') ||
    (hasRole(c, 'manager') && row.gerar_qualificacao_ao_concluir !== 1);
...
async function handleProgressPatch(
  ...
  if (Object.keys(values).length === 0) {
    return errorResponse(c, 400, 'LMS_PROGRESS_INVALID', 'Nenhum campo de progresso foi informado.');
  }
  ...
  if (!existing) return errorResponse(c, 404, 'LMS_ENROLLMENT_NOT_FOUND', 'Matrícula não encontrada.');

  if (!hasRole(c, 'admin', 'manager')) {
    const actorFuncionarioId = await resolveActorFuncionarioId(c, c.env.DB);
    if (!actorFuncionarioId || actorFuncionarioId !== existing.funcionario_id) {
      return errorResponse(c, 403, 'LMS_PROGRESS_OWNERSHIP_REQUIRED', 'Sem permissão para atualizar esta matrícula.');
    }
  }
  if (String(existing.status).toUpperCase() === 'CANCELADO') {
    return errorResponse(c, 409, 'LMS_ENROLLMENT_INACTIVE', 'Matrícula cancelada não aceita progresso.');
  }
  ...
  const newStatus = String(existing.status).toUpperCase() === 'NAO_INICIADO' ? 'EM_ANDAMENTO' : existing.status;
  const setClauses = ["updated_at = datetime('now')", 'status = ?', "data_inicio = COALESCE(data_inicio, datetime('now'))"];
...
async function guardSelfEnrollment(
  ...
  if (existing && (existing.deleted_at || String(existing.status).toUpperCase() === 'CANCELADO')) {
    return errorResponse(
      ...
      { matricula_id: existing.id, rematriculation_endpoint: `/api/lms/matriculas/${existing.id}/rematricular` },
    );
  }
...
async function handleRematriculation(
  ...
  if (reason.length < 10) {
    return errorResponse(c, 400, 'LMS_REMATRICULATION_REASON_REQUIRED', 'Informe uma justificativa com ao menos 10 caracteres.');
  }
  const existing = await c.env.DB
    .prepare(
  ...
  try {
    await c.env.DB.batch([
      ...
      c.env.DB
        ...
        .bind(body.data_expiracao ?? null, `Rematrícula: ${reason}`, `Rematrícula: ${reason}`, matriculaId, empresaId),
      c.env.DB
        .prepare(
    ...
    return errorResponse(c, 409, 'LMS_REMATRICULATION_CONFLICT', 'A rematrícula concorreu com outra alteração. Recarregue o estado.');
  }
  return jsonResponse(c, 200, {
    success: true,
...
async function handleCompletionReversal(
  ...
  if (!hasRole(c, 'admin')) {
    return errorResponse(c, 403, 'LMS_COMPLETION_REVERSAL_FORBIDDEN', 'Somente administradores podem reverter conclusões.');
  }
  const body = await readJsonClone(c);
  const reason = typeof body.reason === 'string' ? body.reason.trim() : '';
  ...
  if (!existing) return errorResponse(c, 404, 'LMS_ENROLLMENT_NOT_FOUND', 'Matrícula não encontrada.');
  if (String(existing.status).toUpperCase() !== 'CONCLUIDO') {
    return errorResponse(c, 409, 'LMS_COMPLETION_NOT_ACTIVE', 'A matrícula não possui conclusão ativa para reverter.');
  }
  ...
  if (existing.qualificacao_historico_id) {
    statements.push(
      c.env.DB
        ...
        .bind(marker, marker, existing.qualificacao_historico_id, empresaId, existing.funcionario_id),
    );
  }
