...
const LOCAL_STATES = new Set<EdbLifecycleStatus>([
  'DRAFT',
...
function parseLifecycleStatus(value: string): EdbLifecycleStatus {
  if (!LOCAL_STATES.has(value as EdbLifecycleStatus)) {
    throw new Error('EDB_REVISION_STATE_INVALID');
  }
  return value as EdbLifecycleStatus;
}
...
function hydrateSignature(row: SignatureRow): EdbSignatureProof {
  if (
    row.tipo !== 'PIC_FLIGHT_RECORD' &&
    row.tipo !== 'OPERATOR_RECORD'
  ) {
    throw new Error('EDB_STORED_SIGNATURE_TYPE_INVALID');
  }
  if (
    !row.id.trim() ||
    !row.revision_id.trim() ||
    !row.signer_nome.trim() ||
    !row.proof_reference.trim()
  ) {
    throw new Error('EDB_STORED_SIGNATURE_INVALID');
  }
  if (!Number.isFinite(Date.parse(row.signed_at))) {
    throw new Error('EDB_STORED_SIGNATURE_TIMESTAMP_INVALID');
  }
...
export async function loadVerifiedEdbRevisionView(params: {
  ...
  if (!row) return null;
  const status = parseLifecycleStatus(row.status);
  ...
  if (!isPersistedEdbFlightRecord(parsed)) {
    throw new Error('EDB_REVISION_PAYLOAD_INVALID');
  }
  ...
  const rows = signatures.results ?? [];
  const picRows = rows.filter((item) => item.tipo === 'PIC_FLIGHT_RECORD');
  const operatorRows = rows.filter((item) => item.tipo === 'OPERATOR_RECORD');
  if (picRows.length > 1 || operatorRows.length > 1) {
    throw new Error('EDB_STORED_SIGNATURE_DUPLICATE_TYPE');
  }
  if (rows.length !== picRows.length + operatorRows.length) {
    throw new Error('EDB_STORED_SIGNATURE_TYPE_INVALID');
  }

  const pic = picRows[0] ? hydrateSignature(picRows[0]) : null;
  if (pic) {
  ...
  const operator = operatorRows[0] ? hydrateSignature(operatorRows[0]) : null;
  if (operator) {
  ...
  if (PIC_REQUIRED_STATES.has(status) && !pic) {
    throw new Error('EDB_PIC_SIGNATURE_REQUIRED_FOR_STATE');
  }
  if (OPERATOR_REQUIRED_STATES.has(status) && !operator) {
