...
export class FuncionarioImportacao {
  ...
  async import(
    ...
    // Helper: Converter data DD/MM/YYYY ou Excel serial para YYYY-MM-DD
    const convertDate = (dateStr: unknown): string | null => {
    ...
    for (let i = 0; i < rows.length; i++) {
      try {
        ...
        // Converter datas
        const nascimento = convertDate(row.Nascimento);
        const admissao = convertDate(row.Admissao);
