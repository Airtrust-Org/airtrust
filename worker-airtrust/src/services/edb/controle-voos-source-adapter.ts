...
function parseHhMmDurationToMinutes(value: string | null): number | null {
  ...
  if (!Number.isSafeInteger(hours) || !Number.isInteger(minutes) || minutes > 59) return null;
...
function assertSourceScope(input: ControleVoosEdbShadowAdapterInput): void {
  ...
  if (rdv.voo_id !== flight.id || rdv.empresa_id !== flight.empresa_id) {
    throw new Error('EDB_SHADOW_RDV_SCOPE_MISMATCH');
  }

  const stageIds = new Set<number>();
  for (const stage of stages) {
    if (stage.voo_id !== flight.id || stage.empresa_id !== flight.empresa_id) {
      throw new Error('EDB_SHADOW_STAGE_SCOPE_MISMATCH');
    }
    if (stageIds.has(stage.id)) {
      throw new Error('EDB_SHADOW_STAGE_DUPLICATED');
    }
    stageIds.add(stage.id);
  }

  for (const member of crew) {
    if (member.empresa_id !== flight.empresa_id) {
      throw new Error('EDB_SHADOW_CREW_TENANT_MISMATCH');
    }
    if (member.voo_id !== flight.id) {
      throw new Error('EDB_SHADOW_CREW_FLIGHT_MISMATCH');
    }
    if (member.etapa_id !== null && !stageIds.has(member.etapa_id)) {
      throw new Error('EDB_SHADOW_CREW_STAGE_MISMATCH');
    }
  }
}

/**
 * Converts already-loaded Controle de Voos/RDV evidence into the normalized
 * read-only source consumed by the regulatory shadow projector.
 *
 * This adapter performs no DB reads/writes. It preserves ambiguous operational
 * values as evidence and never promotes starts to cycles, unclassified IFR to
 * actual/simulated IFR, RDV divergences to technical discrepancies, or RDV
 * summary POB/cargo/cycles into a stage.
 */
