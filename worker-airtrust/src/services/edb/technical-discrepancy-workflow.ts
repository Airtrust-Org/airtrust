...
/**
 * Append-only domain representation for a technical discrepancy attached to
 * one immutable final-record revision. Maintenance actions are appended after
 * detection; they never rewrite the discrepancy evidence on the signed record.
 */
...
function assertPerson(person: EdbPersonIdentity, field: string): void {
...
export function appendReturnToServiceApproval(
  ...
  if (!correctiveAction) {
    throw new Error('Return-to-service approval must reference an existing corrective action');
  }
  ...
  if (
    discrepancy.returnToServiceApprovals.some(
      (item) => item.correctiveActionId === approval.correctiveActionId,
    )
  ) {
...
export function getTechnicalDiscrepancyStatus(
  ...
  if (latest.kind === 'DEFERRED_ACTION_AUTHORIZATION') return 'DEFERRED_ACTION_AUTHORIZED';
