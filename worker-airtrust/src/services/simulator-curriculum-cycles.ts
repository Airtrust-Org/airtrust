...
export async function loadSimulatorCurriculumCycleConfig(params: {
  ...
  const row = await params.db
    .prepare(
      `SELECT qualificacao_tipo_id AS qualification_type_id,
              total_ciclos AS total_cycles,
              ano_base AS base_year,
              ciclo_ano_base AS base_cycle
...
export async function loadResolvedSimulatorCurriculum(params: {
  ...
  if (!(await tableExists(params.db, ITEMS_TABLE))) {
    return {
      ...
      cycle: resolveAnnualCurriculumCycle({
