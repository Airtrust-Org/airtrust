...
**Addendum Sprint Z1 (2026-06-03):** a migration `0387_integracoes_sigvoos_base_tables.sql` foi criada e validada localmente com teste de schema/idempotência. Ela versiona as 3 tabelas base e 4 índices ainda provisionados em runtime por `ensureSigvoosTables()`. **R01 não saiu do runtime nesta sprint.** Motivo: a `0354_auditoria_critica_schema_hardening.sql` já executa `ALTER TABLE integracoes_sigvoos_config` antes de `0387`, então a cadeia limpa de migrations ainda exige tratamento adicional. Novo status: **`MIGRATION_VERSIONED_PENDING_RUNTIME_REMOVAL`**. Restam R01, R04 e R09 no runtime.
...
| Status na matriz | DDL_RUNTIME = PARTIAL (R03 = RESOLVED; R01 = MIGRATION_VERSIONED_PENDING_RUNTIME_REMOVAL (Sprint Z1); restam R04 e R09 no runtime) |

---

**Fim do design document.** Gerado em 2026-06-03. Atualizado com Sprint X.5 closure, Sprint Z0 readiness mapping e Sprint Z1 (`0387` criada, fallback R01 preservado por dependência com `0354`).
