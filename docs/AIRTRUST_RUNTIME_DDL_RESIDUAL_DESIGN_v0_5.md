...
**Addendum Sprint R04.4 (2026-06-03):** a `0388_documentos_canonical_schema.sql` foi versionada localmente junto com teste dedicado de schema/idempotência, sem apply remoto, sem alteração de runtime e sem remoção de `ensureDocumentosTableExists()`. **R04 = MIGRATION_VERSIONED_PENDING_APPLY.**

**Addendum Sprint R04.5 / R01 pós-apply (2026-06-03):** o mecanismo oficial `Cloudflare D1 migrations apply` consumiu a fila pendente em `production` e aplicou `0387_integracoes_sigvoos_base_tables.sql` + `0388_documentos_canonical_schema.sql`. Registrar o desvio como **`APPLY_SCOPE_EXPANDED_BY_PENDING_QUEUE`**: a intenção inicial era aplicar apenas `0388`, mas a fila oficial também continha `0387`. Não houve SQL manual, não houve backfill, não houve consulta de dados de linha, não houve alteração de runtime e não houve deploy. Probe pós-apply: `documentos` com 12 colunas esperadas; índices `idx_documentos_funcionario_tipo`, `idx_documentos_tipo`, `idx_documentos_deleted`, `idx_documentos_funcionario`, `idx_documentos_empresa` e autoíndices SQLite; `pasta_virtual` segue sem `documento_id`; `certificados_templates` sem mudança estrutural observada. Estados consolidados: **`R04 = MIGRATION_APPLIED_PENDING_RUNTIME_REMOVAL`** e **`R01 = 0387_APPLIED_IN_PRODUCTION_BUT_CHAIN_0354_STILL_NEEDS_RECONCILIATION`**.

**Addendum Sprint R04.6 (2026-06-03):** o bootstrap runtime de Documentos foi removido. `auto-migration-documentos.ts` deletado; `api-bootstrap.ts` limpo (import + call removidos); guard test atualizado (`api-bootstrap.ts` fora de `DOCUMENTED_EXCEPTIONS`, R04 documentado como RESOLVED nos comentários). Testes: documentos-canonical-schema (8/8), no-runtime-ddl-hot-paths (13/13), tenant-isolation (12/12) todos PASS. **R04 = RUNTIME_FALLBACK_REMOVED_PENDING_DEPLOY.** Nenhuma migration nova, nenhum schema remoto alterado, nenhum backfill. R01 permanece `0387_APPLIED_IN_PRODUCTION_BUT_CHAIN_0354_STILL_NEEDS_RECONCILIATION`. R09 permanece `RESOLVED`.
>
> **Addendum Sprint R04.7 (2026-06-04):** o deploy do Worker/API foi executado (`APP_VERSION=2026-06-04T01:43:21Z-ca6a7d9`). Smoke pós-deploy: read-only PASS, public-only PASS (3/3). `/api/version` e `/api/health` confirmados funcionais. **R04 = RESOLVED.**
...
| R04 | ~~`utils/auto-migration-documentos.ts`~~ (DELETED) + ~~`runtime/api-bootstrap.ts`~~ (LIMPO) | ~~`ensureDocumentosTableExists()`~~ | ~~CREATE TABLE (1) + CREATE INDEX (5)~~ | `documentos` + 5 índices | RUNTIME_BOOTSTRAP → RESOLVED (Sprint R04.7) | RESOLVED | `0388` aplicada em produção (R04.5). Bootstrap removido: `auto-migration-documentos.ts` deletado; `api-bootstrap.ts` sem import/call; guard test atualizado; 3 suites de teste PASS. Deploy R04.7: APP_VERSION=2026-06-04T01:43:21Z-ca6a7d9, smoke pós-deploy PASS (3/3). | **RESOLVED. Sprint R04.7 (2026-06-04):** deploy + smoke concluídos. |
...
| Resíduos RUNTIME_HOT_PATH confirmados | 1 crítico ativo no runtime atual (R01 SIGVOOS). R03 = RESOLVIDO. R04 = RESOLVED (bootstrap runtime removido + deploy + smoke PASS). R09 = RESOLVIDO. |
...
| Lacunas de migration remanescentes | 1 confirmada (`R01` SIGVOOS base). R04 bootstrap já removido (RESOLVED). |
| Migrations existentes relacionadas | 10+ (`0172`, `0173`, `0183`, `0184`, `0200`, `0280`, `0317`, `0345`, `0352`, `0354`, `0136`-`0138`, `0165`) |
| Migrations novas necessarias | 0 novas para apply imediato; bootstrap de `R04` ja removido; resta reconciliar a cadeia limpa de `R01` |
| Ordem recomendada restante | `R01` baseline/chain plan (único resíduo runtime ativo) |
| Status na matriz | DDL_RUNTIME = PARTIAL (R03 = RESOLVED; R09 = RESOLVED Sprint R09; R04 = RESOLVED Sprint R04.7; R01 = 0387_APPLIED_IN_PRODUCTION_BUT_CHAIN_0354_STILL_NEEDS_RECONCILIATION) |
...
**Addendum Sprint R01 Baseline Strategy (2026-06-03):** estratégia de resolução definida. Editar `0354` rejeitado. Recomendação: `scripts/bootstrap-new-environment.sql` (curto prazo) + squash/rebaseline (longo prazo). `ensureSigvoosTables()` preservado até condições da Seção 9 de `AIRTRUST_SIGVOOS_R01_BASELINE_STRATEGY_v0_5.md` atendidas.

**Fim do design document.** Gerado em 2026-06-03. Atualizado com Sprints X.5, Z0, Z1, Z1.1, R04.2–R04.7, Sprint R01 Chain Reconciliation e Sprint R01 Baseline Strategy (R01 = MIGRATION_APPLIED_CHAIN_RECONCILIATION_REQUIRED; `ensureSigvoosTables()` preservado; próxima fase: R01-bootstrap).
