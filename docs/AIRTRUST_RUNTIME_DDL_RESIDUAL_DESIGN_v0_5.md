...
| R04 | `utils/auto-migration-documentos.ts` + `runtime/api-bootstrap.ts` | `ensureDocumentosTableExists()` | CREATE TABLE (1) + CREATE INDEX (5) | `documentos` + 5 índices (`idx_documentos_funcionario`, `idx_documentos_historico`, `idx_documentos_deleted`, `idx_documentos_r2_key`, `idx_documentos_uuid`) | RUNTIME_BOOTSTRAP → READINESS_MAPPED (Sprint R04.1) | MÉDIO | `0136` reconstrói `documentos` com schema antigo. `0137`/`0138` adicionam índices parciais. `0165` adiciona `empresa_id`. Nenhuma migration canônica única cobre o schema completo atual. | **READINESS_MAPPED. Sprint R04.1 (2026-06-03):** 9 lacunas confirmadas (L1-L9); coluna `historico_id` é bootstrap-only; `sha256_hash`/`empresa_id` são migration-only; migration 0200 referencia colunas fantasmas; probe remoto OBRIGATÓRIO antes da 0388. Ver `docs/AIRTRUST_DOCUMENTOS_DDL_R04_READINESS_v0_5.md`. |
...
**Fim do design document.** Gerado em 2026-06-03. Atualizado com Sprint X.5 closure, Sprint Z0 readiness mapping, Sprint Z1 (`0387` criada) e Sprint Z1.1 (cadeia bloqueada pela `0354`).
