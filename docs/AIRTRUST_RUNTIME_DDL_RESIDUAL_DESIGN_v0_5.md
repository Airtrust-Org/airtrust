...
**Addendum Sprint R04.6 (2026-06-03):** o bootstrap runtime de Documentos foi removido. `auto-migration-documentos.ts` deletado; `api-bootstrap.ts` limpo (import + call removidos); guard test atualizado (`api-bootstrap.ts` fora de `DOCUMENTED_EXCEPTIONS`, R04 documentado como RESOLVED nos comentários). Testes: documentos-canonical-schema (8/8), no-runtime-ddl-hot-paths (13/13), tenant-isolation (12/12) todos PASS. **R04 = RESOLVED.** Nenhuma migration nova, nenhum schema remoto alterado, nenhum backfill. Próximo: deploy + smoke → R04 = RESOLVED. R01 permanece `0387_APPLIED_IN_PRODUCTION_BUT_CHAIN_0354_STILL_NEEDS_RECONCILIATION`. R09 permanece `RESOLVED`.
...
| R04 | ~~`utils/auto-migration-documentos.ts`~~ (DELETED) + ~~`runtime/api-bootstrap.ts`~~ (LIMPO) | ~~`ensureDocumentosTableExists()`~~ | ~~CREATE TABLE (1) + CREATE INDEX (5)~~ | `documentos` + 5 índices | RUNTIME_BOOTSTRAP → RESOLVED (Sprint R04.6) | RESOLVING | `0388` aplicada em produção (R04.5). Bootstrap removido: `auto-migration-documentos.ts` deletado; `api-bootstrap.ts` sem import/call; guard test atualizado; 3 suites de teste PASS. | **RESOLVED. Sprint R04.6 (2026-06-03):** bootstrap removido do runtime. Pendente: deploy + smoke → RESOLVED. |
...
| Resíduos RUNTIME_HOT_PATH confirmados | 1 crítico ativo no runtime atual (R01 SIGVOOS). R03 = RESOLVIDO. R04 = RESOLVED (bootstrap runtime de Documentos removido). R09 = RESOLVIDO. |
