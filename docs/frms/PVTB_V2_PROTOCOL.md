...
It uses the PVT-B stimulus/response approach adapted to AirTrust's operational
sampling window and the familiar computer-PVT visual convention: a black response
field, a fixed red rectangular outline, and a yellow millisecond counter as the
stimulus.
...
| Parameter | Value |
|---|---|
| Session sampling duration | ~1 minute (`60000 ms`) |
...
The one-minute value is the **stimulus sampling window**. No new trial is
...
- a response after the nominal 60-second sampling boundary is accepted when the
...
- `frms_readiness_assessment.protocol_version` (existing column, migration 0472)
  records `airtrust-pvtb-v2` for active sessions.
...
- each crew member builds the 5-session baseline on the active protocol; until
...
- The readiness scoring / classification rules (`deriveReadinessAssessment`) are
  unchanged; the duration change only shortens the operational sampling window.
