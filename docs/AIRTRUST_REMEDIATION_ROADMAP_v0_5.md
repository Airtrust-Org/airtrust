...
**Modo:** Roadmap atualizado após Sprint X.5 (migrations 0385/0386 aplicadas, Worker/API deployado), Sprint Z0 (R01 SIGVOOS readiness mapped) e Sprint R04.5/R01 pós-apply oficial da fila pendente `0387` + `0388` e Sprint R04.6 (bootstrap runtime de Documentos removido). Sem migration manual ou alteração de dados reais.
...
- **Status:** PARTIAL (Sprint V concluído; Sprint W executou a Pré-Fase; Sprint X.4 versionou a M1 de R03 e removeu o fallback runtime localmente; Sprint X.5 aplicou `0386` em produção e deployou o Worker/API. **R03 = RESOLVED.** Sprint Z0 mapeou integralmente R01, a Sprint Z1 criou `0387` + teste local e a Sprint Z1.1 provou o bloqueio de cadeia em `0354`. A Sprint R04.5 registrou `0387` aplicada em produção via fila pendente oficial, mas a cadeia limpa segue pendente. **R01 = 0387_APPLIED_IN_PRODUCTION_BUT_CHAIN_0354_STILL_NEEDS_RECONCILIATION.** Sprint R09 removeu o ALTER TABLE de `shared.ts`. **R09 = RESOLVED.** Sprint R04.2 registrou o probe estrutural remoto de produção para R04, a Sprint R04.3 fechou o desenho documental da `0388`, a Sprint R04.4 versionou a migration e o teste local e a Sprint R04.5 registrou a `0388` aplicada com probe pós-apply. Sprint R04.6 removeu o bootstrap runtime: auto-migration-documentos.ts deletado, api-bootstrap.ts limpo (chamada ensureDocumentosTableExists removida), guard test atualizado (api-bootstrap.ts removido das exceções), 33/33 testes passando. Sprint R04.7 executou o deploy do Worker/API (APP_VERSION=2026-06-04T01:43:21Z-ca6a7d9), smoke pós-deploy PASS (3/3 público, read-only PASS), health + version OK. **R04 = RESOLVED.**).
- **Objetivo:** Concluir as fases remanescentes: Fase 3 (R04 — bootstrap runtime de Documentos removido na R04.6, deploy Worker/API executado e smoke PASS na R04.7; **R04 = RESOLVED**) e Fase 2 (R01 — definir baseline/chain plan limpo apesar da `0387` já aplicada em produção). Fase 1 (R03 Treinamentos Link) concluída. R09 concluído (Sprint R09).
...
- **Escopo:** Sprint W removeu 6 caminhos cobertos sem migration. Sprint X.0 criou o probe estrutural read-only. Sprint X.2 completou o runner remoto read-only. Sprint X.4 registrou probe aprovado em produção para R03, versionou `0386` e removeu o fallback runtime local. **Sprint X.5 aplicou `0386` em produção e deployou o Worker/API (APP_VERSION=2026-06-03T17:00:27Z-c12d8bf).** Sprint Z0 produziu inventário completo de R01, a Sprint Z1 criou `0387_integracoes_sigvoos_base_tables.sql` e a Sprint Z1.1 confirmou localmente que a cadeia limpa falha em `0354` antes da `0387`. A Sprint R04.5 registrou que o mecanismo oficial `Cloudflare D1 migrations apply` consumiu a fila pendente e aplicou `0387` + `0388`, sem SQL manual, sem backfill, sem rows consultadas, sem runtime alterado e sem deploy. Probe pós-apply de R04: índices esperados de `documentos` presentes; `pasta_virtual` segue sem `documento_id`; `certificados_templates` sem mudança estrutural observada. R09 (shared.ts) = RESOLVED (Sprint R09). R04 = RESOLVED.
- **Modelo recomendado:** R04 runtime removal/deploy separado: GPT-5.5 Alta. R01 chain plan: GPT-5.5 Altissimo.
- **Deploy necessário?:** Sim, quando implementado (Fase 1 deploy já feito; Fase 3: bootstrap já removido, deploy pendente).
...
| 7 | ~~DDL Fase 3 - M3 Documentos canonico~~ **CONCLUÍDO** Sprint R04.7 2026-06-04 — R04 = RESOLVED | — | — | — |
...
**Addendum Sprint R01 Chain Reconciliation (2026-06-03):** achado de bloqueio de replay limpo formalizado. `0354` falha em cadeia limpa. `0387` aplicada em produção não resolve cadeia. Testes 8/8 PASS. `ensureSigvoosTables()` preservado. **R01 = MIGRATION_APPLIED_CHAIN_RECONCILIATION_REQUIRED.** Próxima fase: R01-baseline (criar migration de bootstrap idempotente antes de `0354`). Doc de decisão: `docs/AIRTRUST_SIGVOOS_MIGRATION_CHAIN_RECONCILIATION_v0_5.md`.

**Fim do roadmap.** Documento atualizado em 2026-06-03 com Sprint X.5 closure (migrations 0385/0386 aplicadas, Worker/API deployado, R03 = RESOLVED, Audit v2 schema = APPLIED_SCHEMA_READY_FOR_FLAG_PLAN), Sprint Z0 (R01 SIGVOOS readiness mapped), Sprint R04.2, Sprint R04.3, Sprint R04.4, Sprint R04.5/R01 pós-apply oficial (`0387` + `0388`), Sprint R04.6 (bootstrap runtime de Documentos removido), Sprint R04.7 (2026-06-04: deploy Worker/API APP_VERSION=2026-06-04T01:43:21Z-ca6a7d9, smoke pós-deploy PASS 3/3; **R04 = RESOLVED**) e Sprint R01 Chain Reconciliation (**R01 = MIGRATION_APPLIED_CHAIN_RECONCILIATION_REQUIRED**; `ensureSigvoosTables()` preservado).
