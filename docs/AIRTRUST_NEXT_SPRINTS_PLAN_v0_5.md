...
**Addendum Bloco 6.1 Release Gate Residual Reconciliation (2026-06-04):** reconciliação documental completa e schema de produção verificado por read-only. **`PRODUCTION_SCHEMA_STATE = VERIFIED_MISSING_AUDIT_RBAC_SCHEMA`** — `audit_events_v2` presente em produção, tabelas 0389 ausentes. **`CYCLE_CLOSURE = CYCLE_CLOSED_FOR_CONTROLLED_SCOPE_PENDING_RELEASE_GATE`**. **`RELEASE_GATE = READY_WITH_CONDITIONS`**. Próximo bloco recomendado: (1) smoke autenticado com credencial efêmera, avançando para `AUTHENTICATED_SMOKE = PASS`; (2) controlled release/deploy gate execution com snapshot/approval/rollback; (3) se necessário enforcement funcional de RBAC/Suporte em produção, janela separada de apply da `0389` em produção.

**Addendum Bloco 6.2 Authenticated Staging Smoke Gate (2026-06-04):** smoke autenticado efêmero executado e aprovado. **`AUTHENTICATED_SMOKE = PASS` — `PASS=11, FAIL=0, SKIPPED=2`**. Staging remoto acessível e isolado. Usuário de teste pré-existente (empresa_id=1). Sem PII, sem credencial de produção, sem token/cookie persistido. **`RELEASE_GATE = READY_FOR_CONTROLLED_RELEASE`**. `OPS-05 = RESOLVED`. Próximo bloco: **Controlled Release / Deploy Gate Execution** — snapshot pré-deploy, approval nominal, deploy Worker, smoke pós-deploy (público + autenticado), rollback criterion documentado.

**Addendum Controlled Release Execution (2026-06-04):** deploy Worker-only executado em produção com approval `RELEASE-PRODUCTION-20260604-FILIPE`. Versão `2026-06-04T22:35:19Z-d371fd9` deployada via `bash scripts/deploy-worker-safe.sh` (sem migrations, sem DQ/MIG). Smoke público pré-deploy PASS=3, smoke autenticado pré-deploy PASS=11. Smoke público pós-deploy PASS=3 (produção healthy — D1 ok, Storage ok, environment=production). Smoke autenticado pós-deploy PASS=11. Rollback não executado. **`AIRTRUST_RELEASE = DEPLOYED_AND_VALIDATED_FOR_CONTROLLED_SCOPE`**. **`AIRTRUST_AUDIT_HARDENING_CYCLE = CLOSED_WITH_ACCEPTED_RESIDUALS`**. Ciclo de auditoria e hardening encerrado. Próximos passos: monitoramento de produção (24-48h); onboarding de segundo cliente via runbook dedicado; enforcement de RBAC/Suporte v2 em produção via janela separada de apply da `0389`; Audit v2 dual-write em janela controlada futura.
