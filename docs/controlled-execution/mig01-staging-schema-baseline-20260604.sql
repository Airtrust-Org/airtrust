...
CREATE TABLE "_backup_qh_tmp" (
  ...
  deleted_at TEXT,
  FOREIGN KEY(funcionario_id) REFERENCES "funcionarios_backup"(id) ON DELETE RESTRICT
);
...
CREATE TABLE bkp_qual_historico_20260325(
  id INT,
  funcionario_id INT,
  qualificacao_id INT,
  tipo_codigo TEXT,
...
CREATE TABLE bkp_qual_tipos_20260325(
  id INT,
  tipo TEXT,
  codigo TEXT,
  nome TEXT,
...
CREATE TABLE frms_fadiga_checkin (
  ...
  updated_at TEXT NOT NULL DEFAULT (datetime('now')),
  deleted_at TEXT
, jornada_inicio_prevista TEXT, jornada_fim_prevista TEXT, horas_acordado REAL, meds_ult_12h INTEGER NOT NULL DEFAULT 0, alcool_ult_12h INTEGER NOT NULL DEFAULT 0, risco_autoavaliado INTEGER, origem_registro TEXT NOT NULL DEFAULT 'TRIPULANTE');
...
CREATE TABLE habilitacoes (
	...
	updated_at TEXT DEFAULT (datetime('now')),
	deleted_at TEXT
, habilitacao_anterior_id INTEGER, eh_renovada INTEGER DEFAULT 0, renovada_em TEXT);
...
, tipo_sessao_id INTEGER, tipo_aeronave TEXT, codigo_aeronave TEXT, gera_qualificacao BOOLEAN DEFAULT 0, empresa_id INTEGER DEFAULT 1, modelo_aeronave TEXT, qualificacao_tipo_id INTEGER NULL REFERENCES qualificacoes_tipos(id));
...
, lms_matricula_id INTEGER REFERENCES lms_matriculas(id), origem_tipo TEXT
  CHECK(origem_tipo IS NULL OR origem_tipo IN ('LMS', 'PRESENCIAL', 'SIMULADOR', 'IMPORTADO_EDAPP', 'MANUAL')), lms_matricula_ciclo_id INTEGER REFERENCES lms_matricula_ciclos(id));
...
CREATE TABLE "qualificacoes_tipos" (
        ...
        updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        deleted_at DATETIME DEFAULT NULL
      , empresa_id INTEGER DEFAULT 1, is_check INTEGER NOT NULL DEFAULT 0, conteudo_programatico TEXT DEFAULT NULL, carga_horaria_inicial REAL CHECK(carga_horaria_inicial IS NULL OR carga_horaria_inicial > 0), carga_horaria_recorrente REAL CHECK(carga_horaria_recorrente IS NULL OR carga_horaria_recorrente > 0));
...
CREATE TABLE "qualificacoes_tipos_old" (
  id TEXT PRIMARY KEY,
  nome TEXT NOT NULL,
  codigo TEXT NOT NULL,
