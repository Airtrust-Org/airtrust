...
- Riscos cobertos:
  ...
  - negativos de escrita em simuladores (auth/role/erro explícito) e fail-closed de tenant em fichas.
  - guard de auth/tenant/cross-tenant para workflow e listagem de relatos SGSO Next Gen.
- Resultado:
...
- Pendências de cobertura:
  - validação mais forte de política de role em SGSO auditorias/NC e em endpoints SGSO Next Gen extras (`FRAT/MoC`);
  - isolamento cross-tenant explícito em update de sessões simuladores via harness acoplado ao tenant middleware global.
