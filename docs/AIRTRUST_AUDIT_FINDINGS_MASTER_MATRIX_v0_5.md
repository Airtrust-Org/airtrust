...
| ID | Categoria | Achado | Severidade original | Status atual | Correção feita | Commit(s) | Deploy | Evidência/testes | Pendência | Próximo sprint | Modelo recomendado |
|---|---|---|---|---|---|---|---|---|---|---|---|
| MIG-01 | MIGRATION_INTEGRITY | Cadeia canônica de migrations com 30 prefixos duplicados, 3 nomes fora do padrão e exceções históricas de replay/governança | S2 | BLOCKED_BY_CONTROLLED_ENVIRONMENT_NOT_AVAILABLE | Guard local criado para pin de duplicatas, nomes fora do padrão, `CREATE TEMP TABLE` e `PRAGMA foreign_keys = OFF`; Sprint AI adicionou estratégia formal de corte, rollback, dry-run local e readiness scripts sem editar migrations aplicadas; Sprint AK adicionou contrato compartilhado; fechamento final rodou o gate e comprovou bloqueio por ausência de ambiente real | Sprint AH, Sprint AI, Sprint AK, Audit Cycle Final Closure | — | `migration-governance.test.ts`, `sigvoos-base-tables-schema.test.ts`, `readiness-audit-scripts.test.ts`, `controlled-execution-gate.test.ts`, `AIRTRUST_MIGRATION_REBASELINE_READINESS_v0_5.md`, `AIRTRUST_AUDIT_CYCLE_FINAL_CLOSURE_v0_5.md`, `mig01-controlled-rebaseline-gate.sh` | falta target/snapshot/rollback/aprovação/comando revisado; sem isso, rebaseline real permanece bloqueado | Ambiente aprovado + controlled rebaseline | GPT-5.5 Altissimo |
| DQ-01 | DATA_QUALITY | Runner parcial e caminhos críticos de simuladores dependiam de integridade implícita entre tenant, sessão, participante e checks | S1 | BLOCKED_BY_CONTROLLED_ENVIRONMENT_NOT_AVAILABLE | SQL validado estaticamente, runner local seguro criado; OP-1/OP-2 preservados; Sprint AH endureceu caminhos críticos; Sprint AI mapeou riscos; Sprint AJ registrou bloqueio; Sprint AK adicionou contrato; fechamento final rodou o gate e comprovou bloqueio por ausência de ambiente real | `1a9722c`, Sprint AH, Sprint AI, Sprint AJ, Sprint AK, Audit Cycle Final Closure | — | `validate-data-quality-sql.sh` PASS, `simuladores-sessoes-data-quality.test.ts`, `dq01-controlled-backfill-gate.test.ts`, `controlled-execution-gate.test.ts`, `AIRTRUST_DQ01_CONTROLLED_BACKFILL_EXECUTION_v0_5.md`, `AIRTRUST_AUDIT_CYCLE_FINAL_CLOSURE_v0_5.md`, `dq01-controlled-backfill-gate.sh` | falta target/snapshot/rollback/aprovação/comando revisado; sem isso, backfill real permanece bloqueado | Ambiente aprovado + controlled backfill | GPT-5.4 Alta |
