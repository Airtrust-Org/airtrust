...
| DDL-03 | DDL_RUNTIME | `treinamentos-planejados-integration.ts` ainda faz `ALTER TABLE` em `solicitacoes_treinamento` | S2 | BLOCKED_SCHEMA_PROBE_REQUIRED | Sprint V confirmou a lacuna de migration. Sprint X.0 criou probe local. Sprint X.1 tentou autorização (SKIPPED). Sprint X.2 implementou runner remoto read-only com suporte a staging/production via `wrangler d1 execute --remote`. Runner completo e fail-closed; barreira agora é `wrangler login` + env vars. | Sprint X.0, X.1, X.2 | Não | `probe-solicitacoes-treinamento-schema-readonly.sh` (remoto implementado), `AIRTRUST_DDL_M1_SCHEMA_PROBE_EVIDENCE_20260603.md` | Falta evidência estrutural do ambiente aprovado. Runner pronto — operador define env vars + `wrangler login`. | Sprint X.3: executar probe remoto autorizado; decidir M1 | GPT-5.5 Altissimo |
