...
| OPS-05 | OPERACOES_DEPLOY_DB | Smoke autenticado pendente por validação de empresa esperada | P2 | **RESOLVED** | Smoke autenticado executado via `scripts/smoke-auth-terminal-login.sh` contra staging remoto. Login efêmero OK, empresa_id=1 validado. `PASS=11, FAIL=0, SKIPPED=2`. Token/senha não persistidos. `AUTHENTICATED_SMOKE = PASS`. `RELEASE_GATE = READY_FOR_CONTROLLED_RELEASE`. (2026-06-04 Bloco 6.2) | `f614a3f` | — | PASS=11 FAIL=0 SKIPPED=2 — staging remoto isolado, usuário efêmero, sem PII, sem credencial de produção | — | — | Sonnet 4.6 Alto |
...
**Addendum Bloco 6.1 Release Gate Residual Reconciliation (2026-06-04):** reconciliação documental completa. `LOCAL_AUDIT_CLOSURE = COMPLETE_WITH_ENVIRONMENT_BLOCKERS` era stale — status correto: **`CYCLE_CLOSURE = CYCLE_CLOSED_FOR_CONTROLLED_SCOPE_PENDING_RELEASE_GATE`**. Ambiguidade 0385/0389 produção x staging resolvida por read-only SELECT em `sqlite_master` de `airtrust-db`: `audit_events_v2=EXISTS` (0385 aplicada em produção no X.5 ✔), `user_platform_roles=ABSENT`, `support_access_grants=ABSENT`, `support_access_sessions=ABSENT` (0389 somente staging ✔). **`PRODUCTION_SCHEMA_STATE = VERIFIED_MISSING_AUDIT_RBAC_SCHEMA`**. Residuais classificados: `RES-01=ACCEPTED_LOW_RISK_DOCUMENTED`, `DQ-02=BACKLOG_CONTROLLED_SCOPE`, `STATUS-02=BACKLOG_CONTROLLED_SCOPE`, `PERF-01/02/03=REQUIRES_LOAD_TEST_FOR_EXTERNAL_SCALE`. Validações re-executadas nesta sessão: `tsc PASS`, `test:worker PASS (133/874)`, `ops:guard PASS`, `git diff --check PASS`. Smoke autenticado: **`AUTHENTICATED_SMOKE = BLOCKED_BY_MISSING_EPHEMERAL_CREDENTIAL`** (credencial não configurada). **`RELEASE_GATE = READY_WITH_CONDITIONS`** — única condição objetiva restante: smoke autenticado com credencial efêmera. Documento produzido: `AIRTRUST_RELEASE_GATE_RESIDUAL_RECONCILIATION_v0_5.md`. Sem deploy, sem produção mutante, sem migration/apply, sem DQ/MIG.

**Addendum Bloco 6.2 Authenticated Staging Smoke Gate (2026-06-04):** `OPS-05 → RESOLVED`. Smoke autenticado executado via `scripts/smoke-auth-terminal-login.sh` contra staging remoto isolado. Staging acessível (HTTP 200, database staging D1 `b7f50907`). Usuário pré-existente `admin.staging.test@example.invalid` (id=1, empresa_id=1, role=admin, is_primary=1). Senha temporária gerada localmente (bcryptjs salt 10), atualizada no staging D1, não impressa, não commitada. **`AUTHENTICATED_SMOKE = PASS` — `PASS=11, FAIL=0, SKIPPED=2`**. FRMS HTTP 500 = staging sem dados FRMS (endpoint opcional, SKIP correto). `ops:guard PASS`, `tsc PASS`, `test:worker 874/874 PASS`, `git diff --check PASS`. **`RELEASE_GATE = READY_FOR_CONTROLLED_RELEASE`**. Próximo bloco: Controlled Release / Deploy Gate Execution. Sem deploy, sem produção mutante, sem migration/apply, sem DQ/MIG, sem PII, sem token/cookie persistido.

**Addendum Controlled Release Execution (2026-06-04):** deploy Worker-only em produção com approval `RELEASE-PRODUCTION-20260604-FILIPE`. `bash scripts/deploy-worker-safe.sh` — sem migrations, sem DQ/MIG. Versão `2026-06-04T22:35:19Z-d371fd9`, Version ID `60f21802-79ac-495a-9850-a4713fd0463f`. Smoke público pré-deploy PASS=3, smoke autenticado pré-deploy PASS=11, smoke público pós-deploy PASS=3 (produção healthy), smoke autenticado pós-deploy PASS=11. Rollback não executado. **`AIRTRUST_RELEASE = DEPLOYED_AND_VALIDATED_FOR_CONTROLLED_SCOPE`**. **`AIRTRUST_AUDIT_HARDENING_CYCLE = CLOSED_WITH_ACCEPTED_RESIDUALS`**.

**Fim da matriz.** Documento gerado em 2026-06-02. Atualizado com Sprint X.5 closure (R03 = RESOLVED), Sprint R04.7 (**R04 = RESOLVED**), Sprint R01 Chain Reconciliation, Sprint R01 Baseline Strategy, Sprint R01 Bootstrap + Replay Closure, Sprint R01 Staging/New Environment Gate + Fallback Removal Readiness, Sprint R01.4 Runtime Fallback Removal + Final Audit Closure (**R01 = RESOLVED**; `AUDIT_CURRENT_CLOSURE = CLOSED` para o stream R01/DDL residual), Sprint AH (**`MIG-01 = PARTIAL_REQUIRES_FUTURE_REBASELINE` / `DQ-01` parcial com hardening de caminhos críticos**), Sprint AI (**`MIG-01 = READY_FOR_CONTROLLED_REBASELINE` / `DQ-01 = READY_FOR_CONTROLLED_BACKFILL`**), Sprint AJ (**`DQ-01 = BACKFILL_EXECUTION_BLOCKED_BY_ENVIRONMENT_READINESS`** por falta de staging/snapshot/rollback/autorização), Sprint AK (**`MIG-01`/`DQ-01 = READY_FOR_CONTROLLED_EXECUTION_ENVIRONMENT`**), Audit Cycle Final Closure (**`DQ-01`/`MIG-01 = BLOCKED_BY_CONTROLLED_ENVIRONMENT_NOT_AVAILABLE`; `ARCH-01 = MITIGATED_WITH_GUARDS`**), DQ01 Staging Controlled Backfill Execution (**`DQ-01 = RESOLVED_FOR_CONTROLLED_SCOPE`; `MIG-01 = READY_FOR_CONTROLLED_REBASELINE_AFTER_DQ`**), MIG01 Controlled Rebaseline Execution (**`MIG-01 = RESOLVED_FOR_CONTROLLED_SCOPE`; `RBAC_SUPPORT_V2`/`AUDIT_V2 = READY_FOR_CONTROLLED_SCHEMA_MIGRATION`**) e **Controlled Release Execution** (**`AIRTRUST_RELEASE = DEPLOYED_AND_VALIDATED_FOR_CONTROLLED_SCOPE`; `AIRTRUST_AUDIT_HARDENING_CYCLE = CLOSED_WITH_ACCEPTED_RESIDUALS`**).
