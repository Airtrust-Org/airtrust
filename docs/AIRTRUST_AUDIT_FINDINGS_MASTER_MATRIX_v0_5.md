...
| RBAC-02 | RBAC_SUPORTE | Role `support` read-only por tenant inexistente | S1 | LOCAL_FOUNDATION_COMPLETE_BLOCKED_BY_ENVIRONMENT | Sprint P definiu `support_read_only`; Sprint Q fechou a readiness; a etapa atual versionou `0389_platform_roles_support_access_foundation.sql` com grants/sessoes auditaveis e helper local de dual-read | `0389` local-only | — | Docs `AIRTRUST_RBAC_SUPPORT_V2_DESIGN_v0_5.md`, `AIRTRUST_SUPPORT_READ_ONLY_MODEL_v0_5.md`, `AIRTRUST_AUDIT_V2_RBAC_SUPPORT_SCHEMA_READINESS_v0_5.md`, testes `platform-roles-support-access-schema.test.ts` e `platform-access.test.ts` | Falta apply controlado, dual-read em ambiente aprovado e enforcement runtime gradual | Apply controlado + dual-read validado | GPT-5.5 Altissimo |
| RBAC-03 | RBAC_SUPORTE | `platform_admin` persistido ausente | S1 | LOCAL_FOUNDATION_COMPLETE_BLOCKED_BY_ENVIRONMENT | Sprint P definiu o modelo conceitual; a etapa atual versionou a fundacao local `0389` com `user_platform_roles` e helper de resolucao persistida + fallback legado | `0389` local-only | — | Docs `AIRTRUST_PLATFORM_ROLES_MODEL_v0_5.md`, `AIRTRUST_RBAC_V2_MIGRATION_PLAN_v0_5.md`, `AIRTRUST_AUDIT_V2_RBAC_SUPPORT_SCHEMA_READINESS_v0_5.md`, teste `platform-access.test.ts` | Falta apply controlado, dual-read em ambiente aprovado e remocao futura do fallback `userId===1` | Apply controlado + rollout dual-read | GPT-5.5 Altissimo |
| RBAC-04 | RBAC_SUPORTE | Suporte sem audit trail formal próprio | S2 | LOCAL_FOUNDATION_COMPLETE_BLOCKED_BY_ENVIRONMENT | Sprint O documentou `SUPPORT_ACCESS`; a etapa atual versionou `support_access_sessions` e helper pequeno de dual-audit legado + v2 | `0389` local-only | — | Docs `AIRTRUST_SUPPORT_ACCESS_AUDIT_MODEL_v0_5.md`, `AIRTRUST_AUDIT_EVENT_TAXONOMY_v0_5.md`, `AIRTRUST_AUDIT_V2_RBAC_SUPPORT_SCHEMA_READINESS_v0_5.md`, teste `record-legacy-and-canonical-audit.test.ts` | Falta apply controlado, call sites reais de sessao de suporte e ativacao operacional | Apply controlado + integrar sessoes reais | GPT-5.5 Altissimo |
...
| ID | Categoria | Achado | Severidade original | Status atual | Correção feita | Commit(s) | Deploy | Evidência/testes | Pendência | Próximo sprint | Modelo recomendado |
|---|---|---|---|---|---|---|---|---|---|---|---|
| MIG-01 | MIGRATION_INTEGRITY | Cadeia canônica de migrations com 30 prefixos duplicados, 3 nomes fora do padrão e exceções históricas de replay/governança | S2 | LOCAL_READINESS_COMPLETE_BLOCKED_BY_ENVIRONMENT | Guard local criado para pin de duplicatas, nomes fora do padrão, `CREATE TEMP TABLE` e `PRAGMA foreign_keys = OFF`; Sprint AI adicionou estratégia formal de corte, rollback, dry-run local e readiness scripts sem editar migrations aplicadas; Sprint AK adicionou contrato compartilhado; o fechamento local máximo reexecutou o gate e comprovou bloqueio por ausência de ambiente real | Sprint AH, Sprint AI, Sprint AK, Maximum Local Closure | — | `migration-governance.test.ts`, `sigvoos-base-tables-schema.test.ts`, `readiness-audit-scripts.test.ts`, `controlled-execution-gate.test.ts`, `AIRTRUST_MIGRATION_REBASELINE_READINESS_v0_5.md`, `AIRTRUST_MAXIMUM_LOCAL_CLOSURE_REPORT_v0_5.md`, `mig01-controlled-rebaseline-gate.sh` | falta target/snapshot/rollback/aprovação/comando revisado; sem isso, rebaseline real permanece bloqueado | Ambiente aprovado + controlled rebaseline | GPT-5.5 Altissimo |
| DQ-01 | DATA_QUALITY | Runner parcial e caminhos críticos de simuladores dependiam de integridade implícita entre tenant, sessão, participante e checks | S1 | READY_FOR_CONTROLLED_BACKFILL_EXECUTION | SQL validado estaticamente, runner local seguro criado; OP-1/OP-2 preservados; Sprint AH endureceu caminhos críticos; Sprint AI mapeou riscos; Sprint AJ registrou bloqueio; Sprint AK adicionou contrato; Sprint AL materializou pacote `local-copy` com target evidence, snapshot pré-janela, rollback testável e safe command read-only revisado; os gates passaram sem executar mutation | `1a9722c`, Sprint AH, Sprint AI, Sprint AJ, Sprint AK, Sprint AL | — | `validate-data-quality-sql.sh` PASS, `simuladores-sessoes-data-quality.test.ts`, `dq01-controlled-backfill-gate.test.ts`, `controlled-execution-gate.test.ts`, `AIRTRUST_DQ01_CONTROLLED_BACKFILL_EXECUTION_v0_5.md`, `AIRTRUST_DQ01_CONTROLLED_ENVIRONMENT_PACKAGE_v0_5.md`, `dq01-target-evidence-20260604.md`, `dq01-rollback-plan-20260604.md`, `run-dq01-local-copy-backfill-readonly.sh` | backfill real continua não executado; falta aprovar o comando mutante e registrar contagens pré/pós da janela | Executar lote controlado em `local-copy` ou promover pacote equivalente para `staging` | GPT-5.4 Alta |
...
| DQ-01 | DATA_QUALITY | O lote de execução está corretamente bloqueado por falta de staging/snapshot/rollback/autorização explícita | Provisionar ambiente aprovado e rerodar o gate |
