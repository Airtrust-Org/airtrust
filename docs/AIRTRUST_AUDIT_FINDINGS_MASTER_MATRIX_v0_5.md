...
| ID | Categoria | Achado | Severidade original | Status atual | Correção feita | Commit(s) | Deploy | Evidência/testes | Pendência | Próximo sprint | Modelo recomendado |
|---|---|---|---|---|---|---|---|---|---|---|---|
| DDL-01 | DDL_RUNTIME | DDL removido de hot paths (8 funções `ensure*` removidas) | S2 | PARTIAL | Sprint E removeu 8 hot paths. Sprint V inventariou 20 ocorrências. Sprint W executou a Pré-Fase e removeu R02, R05, R06, R07, R08 e R10. | `01f0902`, Sprint V, Sprint W | Sim | `no-runtime-ddl-hot-paths.test.ts` + validações Sprint W | Restam 3 residuais críticos (R01/R03/R04) e R09 (`shared.ts`) | Fase 1-3 (migrar + remover residuais) + revisar R09 | GPT-5.4 Alta / GPT-5.5 Altissimo |
...
| DDL-03 | DDL_RUNTIME | `treinamentos-planejados-integration.ts` ainda faz `ALTER TABLE` em `solicitacoes_treinamento` | S2 | BLOCKED_SCHEMA_PROBE_REQUIRED | Sprint V confirmou a lacuna de migration. Sprint X.0 criou probe local. Sprint X.1 tentou autorização (SKIPPED). Sprint X.2 implementou runner remoto read-only com suporte a staging/production via `wrangler d1 execute --remote`. Sprint X.3 moveu a execução para worktree limpo e reconfirmou `SKIPPED_SCHEMA_PROBE_NOT_AUTHORIZED` com as 4 env vars `UNSET`. Runner completo e fail-closed; a barreira continua operacional (`wrangler login` + env vars). | Sprint X.0, X.1, X.2, X.3 | Não | `probe-solicitacoes-treinamento-schema-readonly.sh` (remoto implementado), `AIRTRUST_DDL_M1_SCHEMA_PROBE_EVIDENCE_20260603.md` | Falta evidência estrutural do ambiente aprovado. Runner pronto — operador autentica no Wrangler e define as env vars. | Sprint X.4: executar probe remoto autorizado; decidir M1 | GPT-5.5 Altissimo |
...
| DDL-03 | DDL_RUNTIME | `treinamentos-planejados-integration.ts` DDL residual | Não (bloqueia 5+ empresas) — `BLOCKED_SCHEMA_PROBE_REQUIRED` |
