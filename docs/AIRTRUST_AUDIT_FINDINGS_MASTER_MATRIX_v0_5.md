...
| ID | Categoria | Achado | Severidade original | Status atual | Correção feita | Commit(s) | Deploy | Evidência/testes | Pendência | Próximo sprint | Modelo recomendado |
|---|---|---|---|---|---|---|---|---|---|---|---|
| DDL-01 | DDL_RUNTIME | DDL removido de hot paths (15 funções/helpers limpos) | S2 | PARTIAL | Sprint E removeu 8 hot paths. Sprint V inventariou 20 ocorrências. Sprint W executou a Pré-Fase e removeu R02, R05, R06, R07, R08 e R10. Sprint X.4 versionou `0386` e removeu o fallback runtime de R03 localmente. Sprint X.5 aplicou `0386` em produção e deployou o Worker/API. Sprint Z1 versionou `0387`. Sprint Z1.1 provou localmente que a cadeia limpa SIGVOOS quebra na `0354`. Sprint R09 removeu o ALTER TABLE de `qualificacoes/shared.ts`; R09 = RESOLVED. Sprint R04.2 registrou o probe estrutural remoto de produção para R04, a Sprint R04.3 fechou o desenho documental da `0388`, a Sprint R04.4 versionou a migration e o teste local e a Sprint R04.5 registrou o apply oficial da fila pendente (`0387` + `0388`) com probe pós-apply. | `01f0902`, Sprint V, Sprint W, Sprint X.4, Sprint X.5, Sprint Z1, Sprint Z1.1, Sprint R09, Sprint R04.2, Sprint R04.3, Sprint R04.4, Sprint R04.5 | Sim (X.5) | `no-runtime-ddl-hot-paths.test.ts`, migration tests `0386`/`0387`, auditoria de cadeia Z1.1, smoke pós-deploy X.5, `qualificacoes-historico-shared-schema.test.ts`, `worker-airtrust/src/__tests__/migrations/documentos-canonical-schema.test.ts`, `AIRTRUST_DOCUMENTOS_DDL_R04_READINESS_v0_5.md`, `AIRTRUST_DOCUMENTOS_0388_CANONICAL_SCHEMA_DESIGN_v0_5.md` | R04 = MIGRATION_APPLIED_PENDING_RUNTIME_REMOVAL. R01 = 0387_APPLIED_IN_PRODUCTION_BUT_CHAIN_0354_STILL_NEEDS_RECONCILIATION. | Fechar bootstrap de Documentos e reconciliação de cadeia de SIGVOOS em sprints separadas | GPT-5.5 Alta / GPT-5.5 Altissimo |
| DDL-02 | DDL_RUNTIME | `sigvoos-frms.ts` ainda cria tabelas `integracoes_sigvoos_*` em runtime | S2 | 0387_APPLIED_IN_PRODUCTION_BUT_CHAIN_0354_STILL_NEEDS_RECONCILIATION | Sprint V confirmou: cobertura de migration apenas parcial (`0352` cobre `sigvoos_mapeamento_manual` + `frms_jornada_pendente`; `0354` referencia `integracoes_sigvoos_config` mas não cria). Sprint Z1 criou `0387_integracoes_sigvoos_base_tables.sql` e teste local. Sprint Z1.1 demonstrou que `0354` falha em cadeia limpa antes de `0387`. Sprint R04.5 registrou `0387` aplicada em produção porque o mecanismo oficial consumiu a fila pendente junto com a `0388`. | Sprint Z1, Sprint Z1.1, Sprint R04.5 | Não | `0387_integracoes_sigvoos_base_tables.sql`, `sigvoos-base-tables-schema.test.ts`, `AIRTRUST_SIGVOOS_MIGRATION_CHAIN_AUDIT_v0_5.md` | Fallback preservado; cadeia limpa ainda exige baseline/plano adicional antes de qualquer remoção do runtime. | Fase 2: baseline/chain plan + depois remoção | GPT-5.5 Altissimo |
...
| DDL-04 | DDL_RUNTIME | `auto-migration-documentos.ts` segue no bootstrap da API | S2 | MIGRATION_APPLIED_PENDING_RUNTIME_REMOVAL (Sprint R04.5) | Sprint R04.1 mapeou o runtime; Sprint R04.2 registrou o probe estrutural remoto real em produção com `PRAGMA` only. Baseline confirmado: `documentos` com `empresa_id DEFAULT 1`, sem `historico_id`/`sha256_hash`, sem índice nominal `idx_documentos_uuid`; `pasta_virtual.documento_id` ausente; `certificados_templates` presente. Sprint R04.3 fechou o desenho conservador da futura `0388`. Sprint R04.4 versionou `0388_documentos_canonical_schema.sql` e o teste dedicado de schema/idempotência. Sprint R04.5 registrou o apply oficial da `0388` e o probe pós-apply PASS estrutural. | Sprint R04.1, Sprint R04.2, Sprint R04.3, Sprint R04.4, Sprint R04.5 | Não | `AIRTRUST_DOCUMENTOS_DDL_R04_READINESS_v0_5.md`, `AIRTRUST_DOCUMENTOS_0388_CANONICAL_SCHEMA_DESIGN_v0_5.md`, `worker-airtrust/src/__tests__/migrations/documentos-canonical-schema.test.ts` | Exige apenas remover bootstrap e deployar em sprint separada; o apply/probe já foram registrados. | Fase 3: remoção do bootstrap de Documentos | GPT-5.5 Alta |
...
| DDL-01 | DDL_RUNTIME | 8 hot paths limpos + R03 fully resolved + R09 resolved + R04 aplicada; 1 residual crítico (R01) | PARTIAL — Sprint X.5: migrations 0385/0386 aplicadas, Worker/API deployado, R03 = RESOLVED. Sprint R09: ALTER TABLE removido de shared.ts, R09 = RESOLVED. Sprint R04.5: R04 = MIGRATION_APPLIED_PENDING_RUNTIME_REMOVAL (baseline remota confirmada, desenho fechado, migration aplicada e probe pós-apply registrado). Resta R01 (0387_APPLIED_IN_PRODUCTION_BUT_CHAIN_0354_STILL_NEEDS_RECONCILIATION). |
...
| DDL-02 | DDL_RUNTIME | `sigvoos-frms.ts` DDL residual | Nao (bloqueia 5+ empresas) - 0387_APPLIED_IN_PRODUCTION_BUT_CHAIN_0354_STILL_NEEDS_RECONCILIATION |
| DDL-03 | DDL_RUNTIME | `treinamentos-planejados-integration.ts` DDL residual | ✅ RESOLVIDO (Sprint X.5) |
| DDL-04 | DDL_RUNTIME | `auto-migration-documentos.ts` DDL residual | Nao (bloqueia 5+ empresas) - MIGRATION_APPLIED_PENDING_RUNTIME_REMOVAL |
...
| DDL-02 | Tabelas `integracoes_sigvoos_*` | CREATE TABLE migrations para 3 tabelas + 4 índices. **Status: 0387_APPLIED_IN_PRODUCTION_BUT_CHAIN_0354_STILL_NEEDS_RECONCILIATION.** `0387` já foi aplicada em produção, mas a cadeia limpa continua inválida porque `0354` antecede a criação da tabela base. |
| DDL-04 | Consolidação de `documentos` em migration canônica | Migration única/idempotente já aplicada; pendente apenas remover o auto-bootstrap sobre a baseline remota capturada |
...
6. **R04 remover o bootstrap de Documentos agora que `0388` já foi aplicada e sondada** - GPT-5.5 Alta (MIGRATION_APPLIED_PENDING_RUNTIME_REMOVAL Sprint R04.5)
7. **Remover o bootstrap de Documentos após smoke funcional da fase de runtime/deploy** - GPT-5.4 Alta
8. **R01 SIGVOOS baseline/chain plan antes de qualquer remoção do fallback** - GPT-5.5 Altissimo
