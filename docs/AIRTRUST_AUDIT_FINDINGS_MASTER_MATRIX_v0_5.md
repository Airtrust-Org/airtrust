...
| DDL-03 | DDL_RUNTIME | `treinamentos-planejados-integration.ts` ainda faz `ALTER TABLE` em `solicitacoes_treinamento` | S2 | BLOCKED_SCHEMA_PROBE_REQUIRED | Sprint V confirmou a lacuna de migration. Sprint X.0 criou probe read-only fail-closed e executou snapshot local: tabela existe, mas colunas/índice ausentes localmente. Sprint X.1 (HEAD `c09c0cb`) tentou executar com autorização mas as 4 env vars permanecem UNSET → `SKIPPED_SCHEMA_PROBE_NOT_AUTHORIZED`. | Sprint X.0, Sprint X.1 | Não | `probe-solicitacoes-treinamento-schema-readonly.sh`, `AIRTRUST_DDL_M1_SCHEMA_PROBE_EVIDENCE_20260603.md` | Falta evidência estrutural do ambiente aprovado para decidir se a M1 é simples, index-only ou desnecessária. | Autorizar probe read-only em staging/produção (Sprint X.2); depois decidir M1 | GPT-5.5 Altissimo |
