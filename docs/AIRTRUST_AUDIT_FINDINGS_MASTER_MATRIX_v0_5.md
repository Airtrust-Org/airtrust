...
| ID | Categoria | Achado | Severidade original | Status atual | Correção feita | Commit(s) | Deploy | Evidência/testes | Pendência | Próximo sprint | Modelo recomendado |
|---|---|---|---|---|---|---|---|---|---|---|---|
| DDL-01 | DDL_RUNTIME | DDL removido de hot paths (15 funções/helpers limpos) | S2 | RESOLVED | Sprint E removeu 8 hot paths. Sprint V inventariou 20 ocorrências. Sprint W executou a Pré-Fase e removeu R02, R05, R06, R07, R08 e R10. Sprint X.4 versionou `0386` e removeu o fallback runtime de R03 localmente. Sprint X.5 aplicou `0386` em produção e deployou o Worker/API. Sprint Z1 versionou `0387`. Sprint Z1.1 provou localmente que a cadeia limpa SIGVOOS quebra na `0354`. Sprint R01.2 criou o bootstrap local de novo ambiente e provou o replay controlado sem tocar D1 remoto. Sprint R09 removeu o ALTER TABLE de `qualificacoes/shared.ts`; R09 = RESOLVED. Sprint R04.2 registrou o probe estrutural remoto de produção para R04, a Sprint R04.3 fechou o desenho documental da `0388`, a Sprint R04.4 versionou a migration e o teste local e a Sprint R04.5 registrou o apply oficial da fila pendente (`0387` + `0388`) com probe pós-apply. Sprint R04.6 removeu o bootstrap runtime: auto-migration-documentos.ts deletado, api-bootstrap.ts limpo, guard test atualizado, testes (33/33) PASS. Sprint R04.7 executou o deploy do Worker/API (APP_VERSION=2026-06-04T01:43:21Z-ca6a7d9), smoke pós-deploy PASS (3/3 público, read-only PASS). R04 = RESOLVED. | `01f0902`, Sprint V, Sprint W, Sprint X.4, Sprint X.5, Sprint Z1, Sprint Z1.1, Sprint R01.2, Sprint R09, Sprint R04.2, Sprint R04.3, Sprint R04.4, Sprint R04.5, Sprint R04.6, Sprint R04.7 | Sim (X.5, R04.7) | `no-runtime-ddl-hot-paths.test.ts`, migration tests `0386`/`0387`, auditoria de cadeia Z1.1, `scripts/bootstrap-new-environment.sql`, smoke pós-deploy X.5 e R04.7, `qualificacoes-historico-shared-schema.test.ts`, `worker-airtrust/src/__tests__/migrations/documentos-canonical-schema.test.ts`, `AIRTRUST_DOCUMENTOS_DDL_R04_READINESS_v0_5.md`, `AIRTRUST_DOCUMENTOS_0388_CANONICAL_SCHEMA_DESIGN_v0_5.md` | R01 = BOOTSTRAP_IMPLEMENTED_RUNTIME_FALLBACK_PENDING_REMOVAL_GATE. R04 = RESOLVED. | Gate de ambiente novo/staging para SIGVOOS (R01) em sprint separada | GPT-5.5 Alta / GPT-5.5 Altissimo |
| DDL-02 | DDL_RUNTIME | `sigvoos-frms.ts` ainda cria tabelas `integracoes_sigvoos_*` em runtime | S2 | BOOTSTRAP_IMPLEMENTED_RUNTIME_FALLBACK_PENDING_REMOVAL_GATE | Sprint V confirmou: cobertura de migration apenas parcial (`0352` cobre `sigvoos_mapeamento_manual` + `frms_jornada_pendente`; `0354` referencia `integracoes_sigvoos_config` mas não cria). Sprint Z1 criou `0387_integracoes_sigvoos_base_tables.sql` e teste local. Sprint Z1.1 demonstrou que `0354` falha em cadeia limpa antes de `0387`. Sprint R04.5 registrou `0387` aplicada em produção porque o mecanismo oficial consumiu a fila pendente junto com a `0388`. Sprint R01.2 criou `scripts/bootstrap-new-environment.sql`, o runbook operacional e a prova local de replay com bootstrap antes da cadeia histórica. | Sprint Z1, Sprint Z1.1, Sprint R04.5, Sprint R01.2 | Não | `0387_integracoes_sigvoos_base_tables.sql`, `scripts/bootstrap-new-environment.sql`, `sigvoos-base-tables-schema.test.ts`, `AIRTRUST_SIGVOOS_MIGRATION_CHAIN_AUDIT_v0_5.md`, `AIRTRUST_SIGVOOS_R01_NEW_ENVIRONMENT_BOOTSTRAP_AND_REPLAY_CLOSURE_v0_5.md` | Fallback preservado; a cadeia limpa agora tem bootstrap local documentado, mas a remoção do runtime ainda depende de validação controlada em ambiente novo aprovado. | Fase 2: gate de ambiente novo/staging + depois remoção | GPT-5.5 Altissimo |
...
| DDL-01 | DDL_RUNTIME | 8 hot paths limpos + R03 fully resolved + R09 resolved + R04 resolved; 1 residual crítico (R01) | RESOLVED — Sprint X.5: migrations 0385/0386 aplicadas, Worker/API deployado, R03 = RESOLVED. Sprint R09: ALTER TABLE removido de shared.ts, R09 = RESOLVED. Sprint R04.6: bootstrap removido (auto-migration-documentos.ts deletado, api-bootstrap.ts limpo, guard test atualizado, testes 33/33 PASS). Sprint R04.7: deploy Worker/API (APP_VERSION=2026-06-04T01:43:21Z-ca6a7d9), smoke pós-deploy PASS (3/3 público, read-only PASS), R04 = RESOLVED. Resta R01 (BOOTSTRAP_IMPLEMENTED_RUNTIME_FALLBACK_PENDING_REMOVAL_GATE). |
...
| DDL-02 | DDL_RUNTIME | `sigvoos-frms.ts` DDL residual | Nao (bloqueia 5+ empresas) - BOOTSTRAP_IMPLEMENTED_RUNTIME_FALLBACK_PENDING_REMOVAL_GATE |
...
| DDL-02 | Tabelas `integracoes_sigvoos_*` | Bootstrap local + migrations históricas para 3 tabelas + 4 índices. **Status: BOOTSTRAP_IMPLEMENTED_RUNTIME_FALLBACK_PENDING_REMOVAL_GATE.** `0387` já foi aplicada em produção, `scripts/bootstrap-new-environment.sql` já existe para ambientes novos, e a cadeia limpa só fica válida quando o bootstrap roda antes da `0354`. |
...
**Fim da matriz.** Documento gerado em 2026-06-02. Atualizado com Sprint X.5 closure (R03 = RESOLVED), Sprint R04.7 (**R04 = RESOLVED**), Sprint R01 Chain Reconciliation, Sprint R01 Baseline Strategy e Sprint R01 Bootstrap + Replay Closure (R01 = BOOTSTRAP_IMPLEMENTED_RUNTIME_FALLBACK_PENDING_REMOVAL_GATE; próxima fase: R01-staging/new-environment gate).
