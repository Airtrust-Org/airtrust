...
**Sprint:** V/W/X.0–X.5 / R04.7 — DDL Runtime Residual Design + Schema Probe + Apply/Deploy + Bootstrap Final Removal + Deploy/Resolve
**Modo:** Read-only / docs+runner-only até Sprint X.4. Sprint X.5 executou apply de migrations + deploy do Worker/API. Sprint R04.5 registrou o apply oficial da fila pendente `0387` + `0388`, sem SQL manual e sem deploy adicional. Sprint R04.6 removeu o bootstrap runtime: `auto-migration-documentos.ts` deletado, `api-bootstrap.ts` limpo, guard test atualizado. Sprint R04.7 executou deploy Worker/API + smoke pós-deploy PASS. **R04 = RESOLVED.**
...
**Addendum Sprint R04.6 (2026-06-03):** o bootstrap runtime de Documentos foi removido. Ações executadas: (1) `auto-migration-documentos.ts` deletado do runtime — `git rm src/utils/auto-migration-documentos.ts`; (2) `api-bootstrap.ts` limpo — `import { ensureDocumentosTableExists }` e `await ensureDocumentosTableExists(db)` removidos; (3) guard test `no-runtime-ddl-hot-paths.test.ts` atualizado — `api-bootstrap.ts` removido de `DOCUMENTED_EXCEPTIONS` e R04 documentado como `RESOLVED` nos comentários do teste; (4) `documentos-canonical-schema` (8/8), `no-runtime-ddl-hot-paths` (13/13) e `tenant-isolation` (12/12) todos PASS. **Novo status consolidado: `R04 = RUNTIME_FALLBACK_REMOVED_PENDING_DEPLOY`.** Nenhuma migration nova, nenhuma alteração de schema remoto, nenhum backfill, nenhuma mudança de dados. Próxima ação: deploy do Worker/API → smoke pós-deploy → R04 = RESOLVED. R01 permanece `0387_APPLIED_IN_PRODUCTION_BUT_CHAIN_0354_STILL_NEEDS_RECONCILIATION`. R09 permanece `RESOLVED`.
>
...
**Addendum Sprint R01 Chain Reconciliation (2026-06-03):** achado de bloqueio de replay limpo formalizado. Nenhuma migration anterior à `0354` cria `integracoes_sigvoos_config`. `0354` falha em cadeia limpa; `0387` aplicada em produção não resolve replay. Testes locais 8/8 PASS. `ensureSigvoosTables()` preservado. **R01 = MIGRATION_APPLIED_CHAIN_RECONCILIATION_REQUIRED.** Doc de decisão: `docs/AIRTRUST_SIGVOOS_MIGRATION_CHAIN_RECONCILIATION_v0_5.md`. DDL runtime remanescente: apenas R01.

**Addendum Sprint R01 Baseline Strategy (2026-06-03):** estratégia de resolução definida. Editar `0354` rejeitado. `0389` isolada insuficiente. Recomendação curto prazo: `scripts/bootstrap-new-environment.sql`. Recomendação longo prazo: squash/rebaseline. `ensureSigvoosTables()` preservado. Doc de estratégia: `docs/AIRTRUST_SIGVOOS_R01_BASELINE_STRATEGY_v0_5.md`.

**Fim do readiness document.** Gerado em 2026-06-03. Atualizado com Sprints R09, R04.1–R04.4, R04.5, R04.6, R04.7 (**R04 = RESOLVED**), Sprint R01 Chain Reconciliation e Sprint R01 Baseline Strategy (R01 = MIGRATION_APPLIED_CHAIN_RECONCILIATION_REQUIRED; `ensureSigvoosTables()` preservado; próxima fase: R01-bootstrap).
