...
| `POST /api/controle-voos/voos/:id/rdv/finalizar-preenchimento` | `versao` canônica obrigatória | RDV `preenchimento_finalizado` com `versao + 1` | `student` atribuido ou Coordenação por capability | vínculo/tenant; CAS otimista; confirmação operacional sem assinatura | batch atômico: `cv_rdv_operacional` + `cv_voo_eventos` + aprovação `COMANDANTE`; auditoria UPDATE |
