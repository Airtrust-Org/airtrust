...
| `MIG-01` - Migration Integrity | READY_FOR_CONTROLLED_EXECUTION_ENVIRONMENT | diretorio canonico segue com 30 prefixos duplicados, 3 nomes fora do padrao e excecoes historicas de replay, mas agora existe estrategia formal de corte, rollback, contrato compartilhado e gate de execucao controlada | vincular target/janela aprovados e executar a janela de rebaseline em ambiente aprovado | GPT-5.5 Altissimo |
...
| Data Quality | READY_FOR_CONTROLLED_EXECUTION_ENVIRONMENT | checks continuam sem saneamento real, mas agora existe contrato compartilhado, gate fail-closed e runbook para liberar uma janela controlada futura | vincular target/janela aprovados e executar o gate de backfill antes da mutation real | GPT-5.4 Alta |
