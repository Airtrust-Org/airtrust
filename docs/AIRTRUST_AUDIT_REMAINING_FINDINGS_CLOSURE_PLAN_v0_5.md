...
|---|---|---|---|---|
| `R01` - SIGVOOS runtime DDL | BOOTSTRAP_IMPLEMENTED_RUNTIME_FALLBACK_PENDING_REMOVAL_GATE | `0387` já foi aplicada em produção e o bootstrap local para ambiente novo foi criado/testado, mas o fallback runtime ainda precisa permanecer até validação controlada em ambiente novo aprovado | validar bootstrap/runbook antes de qualquer remoção do fallback | GPT-5.5 Altissimo |
...
6. `R01 SIGVOOS Bootstrap/New Environment Gate` — único resíduo runtime ativo. Validar o bootstrap/runbook em ambiente novo aprovado antes de remover o fallback R01. A `0387` já está aplicada em produção e o bootstrap local já foi implementado/testado.
...
4. `R01` nao depender mais de DDL runtime (único resíduo runtime ativo). Nesta fase, o status correto avancou para `BOOTSTRAP_IMPLEMENTED_RUNTIME_FALLBACK_PENDING_REMOVAL_GATE`: o bootstrap local existe e foi provado, mas `ensureSigvoosTables()` segue preservado ate o gate em ambiente novo aprovado. `R04` = RESOLVED (Sprint R04.6 + R04.7: bootstrap removido, deploy + smoke PASS). `R09` = RESOLVED (Sprint R09, 2026-06-03).
