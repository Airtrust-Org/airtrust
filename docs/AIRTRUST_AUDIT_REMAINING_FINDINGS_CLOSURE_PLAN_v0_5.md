...
| Data Quality | READY_FOR_CONTROLLED_BACKFILL_EXECUTION | pacote `local-copy` rastreável criado; target, snapshot, rollback, approval e safe command revisado agora existem e os gates passaram sem mutação | aprovar a janela e executar o lote controlado de `DQ-01`, ou materializar pacote equivalente para `staging` | GPT-5.4 Alta |
