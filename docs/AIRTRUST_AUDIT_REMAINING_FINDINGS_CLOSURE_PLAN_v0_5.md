...
| `MIG-01` - Migration Integrity | WAITING_FOR_DQ_STAGING_OR_CONTROLLED_DECISION | DQ-01 tem execução local-copy validada, mas ainda não staging; rebaseline não deve iniciar sem decisão explícita | executar DQ em staging aprovado ou registrar decisão controlada para liberar MIG em janela separada | GPT-5.5 Altissimo |
...
| Data Quality | LOCAL_COPY_BACKFILL_VALIDATED_READY_FOR_STAGING | backfill local-copy executado para alinhamento seguro de soft delete: 17 candidatos, 17 alterados, 0 remanescentes; ainda há WARNs e SKIPPEDs que exigem staging/schema completo | promover diagnóstico/backfill para staging aprovado antes de encerrar DQ como resolvido | GPT-5.4 Alta |
