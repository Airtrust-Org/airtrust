...
>
> **Addendum Sprint R04.6 (2026-06-03):** o bootstrap runtime foi completamente removido — `auto-migration-documentos.ts` deletado, `api-bootstrap.ts` limpo, guard test atualizado. Migration `0388` confirmada como aplicada em produção. Todos os 33/33 testes passam. Novo status consolidado: **`R04 = RESOLVED`**. Próximo passo: deployar Worker/API, smoke funcional, então `R04 = RESOLVED`.
>
> **Addendum Sprint R04.7 (2026-06-04):** o deploy do Worker/API foi executado (`APP_VERSION=2026-06-04T01:43:21Z-ca6a7d9`). Smoke pós-deploy: read-only PASS, public-only PASS (3/3). `/api/version` retorna a versão correta, `/api/health` saudável. **R04 = RESOLVED.**
