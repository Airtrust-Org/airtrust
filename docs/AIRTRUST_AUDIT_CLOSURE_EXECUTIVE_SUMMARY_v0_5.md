...
| **DDL Runtime** | 15 hot paths/helpers limpos, guard endurecido | Sprint V inventariou 20 ocorrências; Sprint W removeu os 6 caminhos cobertos (R02, R05, R06, R07, R08, R10); Sprint X.4 versionou `0386` e removeu o fallback de R03; Sprint X.5 aplicou `0386` em produção e deployou o Worker/API. R03 = RESOLVED. Sprint Z0 mapeou integralmente R01 (SIGVOOS), Sprint Z1 criou `0387` e Sprint Z1.1 provou a falha da cadeia limpa na `0354`. Sprint R04.5 registrou que o mecanismo oficial `Cloudflare D1 migrations apply` consumiu a fila pendente e aplicou `0387` + `0388` em produção, sem SQL manual, sem backfill, sem consulta de dados de linha e sem deploy. R01 = 0387_APPLIED_IN_PRODUCTION_BUT_CHAIN_0354_STILL_NEEDS_RECONCILIATION. R04 = RESOLVED. Sprint R04.6 removeu o bootstrap runtime (auto-migration-documentos.ts deletado, api-bootstrap.ts limpo, guard test atualizado, 33/33 testes passando). Sprint R04.7 executou o deploy do Worker/API (APP_VERSION=2026-06-04T01:43:21Z-ca6a7d9), smoke pós-deploy PASS (3/3 público, read-only PASS). R04 = RESOLVED. Sprint R09 removeu o ALTER TABLE de `shared.ts`; R09 = RESOLVED. |
...
6. **DDL runtime residual** em SIGVOOS e documentos. R03 = RESOLVED (Sprint X.5: `0386` aplicada + deploy). R09 = RESOLVED (Sprint R09: ALTER TABLE removido de `shared.ts`). R04 = RESOLVED (Sprint R04.7: deploy Worker/API + smoke pós-deploy PASS). R01 = 0387_APPLIED_IN_PRODUCTION_BUT_CHAIN_0354_STILL_NEEDS_RECONCILIATION.
...
5. **Fechar o bloco DDL residual restante** na ordem `R04 -> R01`, mantendo a abordagem conservadora para schema/migrations. R09 = RESOLVED (Sprint R09). R04 = RESOLVED (Sprint R04.7: deploy Worker/API + smoke pós-deploy PASS). Agora atacar a reconciliação da cadeia limpa de SIGVOOS (R01).
...
**Fim do resumo executivo.** Documento gerado em 2026-06-02. Atualizado em 2026-06-03 com Sprint X.5 closure (migrations 0385/0386 aplicadas em produção, Worker/API deployado, APP_VERSION=2026-06-03T17:00:27Z-c12d8bf, smoke pós-deploy PASS, R03=RESOLVED), Sprint R04.5/R01 pós-apply oficial da fila pendente `0387` + `0388`, Sprint R04.6 (bootstrap runtime de Documentos removido — auto-migration-documentos.ts deletado, api-bootstrap.ts limpo, todos os 33 testes passando; R04 = RESOLVED; R01 = 0387_APPLIED_IN_PRODUCTION_BUT_CHAIN_0354_STILL_NEEDS_RECONCILIATION) e Sprint R04.7 (deploy Worker/API APP_VERSION=2026-06-04T01:43:21Z-ca6a7d9, smoke pós-deploy read-only PASS / public-only PASS 3/3, health + version OK; R04 = RESOLVED).
