...
| **Data Quality** | SQL validado, runner local criado, 10 checks executados (5 PASS, 4 WARN, 5 SKIPPED); OP-1 repetiu a evidencia local com o mesmo perfil agregado | Executar em ambiente com schema completo para zerar SKIPPED |
| **DDL Runtime** | 15 hot paths/helpers limpos, guard endurecido | Sprint V inventariou 20 ocorrências; Sprint W removeu os 6 caminhos cobertos (R02, R05, R06, R07, R08, R10); Sprint X.4 versionou `0386` e removeu o fallback de R03; Sprint X.5 aplicou `0386` em produção e deployou o Worker/API. R03 = RESOLVED. Sprint Z0 mapeou integralmente R01 (SIGVOOS), Sprint Z1 criou `0387` e Sprint Z1.1 provou a falha da cadeia limpa na `0354`. R01 = MIGRATION_CHAIN_BLOCKED_BY_0354. Restam 1 residual crítico no runtime (R04) e 1 caso dinâmico incerto (`shared.ts`) |
...
5. **Smoke autenticado**: validacao funcional historica existe (PASS=11/11), mas a sessao OP-1 ficou `SKIPPED_AUTH_REQUIRED` por ausencia de credencial e de `AIRTRUST_EXPECTED_EMPRESA_ID`/`CODIGO`.

### Não bloqueadores (para piloto interno)

6. **DDL runtime residual** em SIGVOOS, documentos e `shared.ts`. R03 = RESOLVED (Sprint X.5: `0386` aplicada + deploy). R01 = MIGRATION_CHAIN_BLOCKED_BY_0354 (Sprint Z1.1: prova local da falha de cadeia limpa antes da `0387`). Restam 1 residual crítico no runtime (R04) + 1 caso dinâmico (R09).
...
**Decisao operacional OP-1:** `CONDITIONAL GO`.
