...
| **Migration Integrity** | Sprint AH congelou a governança local, Sprint AI fechou a estratégia de rebaseline controlado com doc/rollback/dry-run local e Sprint AK adicionou contrato/runbook/gate compartilhados | Falta apenas vincular target/janela aprovados e executar a janela controlada |
| **Data Quality** | SQL validado, runner local criado, 10 checks executados (5 PASS, 4 WARN, 5 SKIPPED); Sprint AH endureceu caminhos críticos de simuladores, Sprint AI fechou o mapa de backfill, Sprint AJ registrou o bloqueio operacional e Sprint AK adicionou contrato/runbook/gate compartilhados | A camada de execução controlada está pronta, mas ainda falta vincular target/janela aprovados antes da mutation real |
