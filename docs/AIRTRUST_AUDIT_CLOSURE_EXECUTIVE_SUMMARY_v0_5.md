...
**Addendum Bloco 6.2 Authenticated Staging Smoke Gate (2026-06-04):** smoke autenticado efêmero executado via `scripts/smoke-auth-terminal-login.sh` contra staging remoto (`airtrust-api-staging.airtrust.workers.dev`). Staging D1 isolado confirmado (database_id `b7f50907-c110-45f5-ad17-e97ea47f2826`, separado de produção). Usuário de teste pré-existente `admin.staging.test@example.invalid` (id=1, perfil=ADMIN, empresa_id=1, is_primary=1). Senha temporária gerada localmente via bcryptjs (salt 10), aplicada no staging D1, não persistida, não impressa, não commitada. Login efêmero OK. **`AUTHENTICATED_SMOKE = PASS`** — `PASS=11, FAIL=0, SKIPPED=2` (FRMS HTTP 500 = staging sem dados FRMS, endpoint opcional; FRMS fail-safe = não habilitado). `ops:guard PASS`, `tsc PASS`, `test:worker 874/874 PASS`. **`RELEASE_GATE = READY_FOR_CONTROLLED_RELEASE`**. Próximo bloco: Controlled Release / Deploy Gate Execution. Sem deploy, sem produção mutante, sem migration/apply, sem PII, sem token/cookie persistido.

**Addendum Pre-Release Product Fix Pack (2026-06-04):** fix pack funcional aplicado sem deploy, sem produção mutante, sem migration/apply e sem DQ/MIG. Correções: notificação por e-mail de sessão de simulador para instrutor/tripulantes; preservação da visualização semanal/mensal; pós-salvamento de fadiga diária com home correta por perfil e proteção contra double submit; impressão A4 landscape da agenda; ícones de pasta apontando para Pasta 360. Testes específicos e `tsc` worker passaram. Como o payload funcional mudou após o smoke de Bloco 6.2, status operacional pós-fix: **`PRE_RELEASE_PRODUCT_FIXES = COMPLETE`** e **`RELEASE_GATE = READY_FOR_CONTROLLED_RELEASE_PENDING_FINAL_SMOKE_RECHECK`**. Doc: `AIRTRUST_PRE_RELEASE_PRODUCT_FIX_PACK_v0_5.md`.

**Addendum Controlled Release Execution (2026-06-04):** deploy Worker-only executado em produção. Approval: `RELEASE-PRODUCTION-20260604-FILIPE`. Comando: `bash scripts/deploy-worker-safe.sh` (sem migrations, sem DQ/MIG, sem Pages). Versão deployada: `2026-06-04T22:35:19Z-d371fd9`. Cloudflare Version ID: `60f21802-79ac-495a-9850-a4713fd0463f`. Smoke público pré-deploy PASS=3. Smoke autenticado pré-deploy PASS=11. Smoke público pós-deploy PASS=3 (`api.airtrust.online`, environment=production, D1 ok, Storage ok). Smoke autenticado pós-deploy PASS=11. Rollback não executado. Sem migrations, sem DQ/MIG, sem secrets/PII. **`AIRTRUST_RELEASE = DEPLOYED_AND_VALIDATED_FOR_CONTROLLED_SCOPE`**. **`AIRTRUST_AUDIT_HARDENING_CYCLE = CLOSED_WITH_ACCEPTED_RESIDUALS`**. Documento: `AIRTRUST_CONTROLLED_RELEASE_EXECUTION_RESULT_v0_5.md`.
