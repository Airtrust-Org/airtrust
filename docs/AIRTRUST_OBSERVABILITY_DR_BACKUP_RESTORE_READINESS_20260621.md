...
Worktree: `/tmp/airtrust-observability-dr-readiness`
...
- O worker expoe `/api/health`, `/api/version` e `/api/status` com no-cache em [worker-airtrust/src/routes/system.ts](/tmp/airtrust-observability-dr-readiness/worker-airtrust/src/routes/system.ts:65).
- O worker aplica `X-Request-ID` via [worker-airtrust/src/middleware/requestId.ts](/tmp/airtrust-observability-dr-readiness/worker-airtrust/src/middleware/requestId.ts:1) e retorna `requestId` em erros via [worker-airtrust/src/middleware/error-handler.ts](/tmp/airtrust-observability-dr-readiness/worker-airtrust/src/middleware/error-handler.ts:13).
...
- O backend possui servicos e rotas versionadas para backup e restore em [worker-airtrust/src/routes/backup.ts](/tmp/airtrust-observability-dr-readiness/worker-airtrust/src/routes/backup.ts:1), [worker-airtrust/src/services/backup/orchestrator.ts](/tmp/airtrust-observability-dr-readiness/worker-airtrust/src/services/backup/orchestrator.ts:1) e [worker-airtrust/src/services/backup/restore.ts](/tmp/airtrust-observability-dr-readiness/worker-airtrust/src/services/backup/restore.ts:1).
...
- `X-AirTrust-Version` no middleware global em [worker-airtrust/src/index.ts](/tmp/airtrust-observability-dr-readiness/worker-airtrust/src/index.ts:145);
...
- o script [scripts/analyze-logs.sh](/tmp/airtrust-observability-dr-readiness/scripts/analyze-logs.sh:1) estava consumindo `wrangler tail` sem gate proprio e repassando campos brutos de log; isso foi endurecido nesta macroetapa para exigir gate operacional e suprimir `userEmail`, `message` e `error` brutos por padrao;
...
- cron de backup diario/semanal/mensal em [worker-airtrust/src/cron/scheduled-handler.ts](/tmp/airtrust-observability-dr-readiness/worker-airtrust/src/cron/scheduled-handler.ts:420);
- orquestracao com checksum/manifest no R2 em [worker-airtrust/src/services/backup/orchestrator.ts](/tmp/airtrust-observability-dr-readiness/worker-airtrust/src/services/backup/orchestrator.ts:511);
- manifesto e evidencias locais em `docs/production-backup/` e `docs/backup-readiness/`;
- script legado remoto [scripts/backup_d1_to_r2.sh](/tmp/airtrust-observability-dr-readiness/scripts/backup_d1_to_r2.sh:1) ja bloqueado por fail-closed.

### Lacunas

- o script [scripts/backup-database.sh](/tmp/airtrust-observability-dr-readiness/scripts/backup-database.sh:1) e historico, mistura fluxos, tem limpeza final inconsistente e nao deve ser tratado como runbook canonico;
...
- gates fortes para deploy e migration em [scripts/deploy-worker-only.sh](/tmp/airtrust-observability-dr-readiness/scripts/deploy-worker-only.sh:1) e [scripts/deploy-worker-safe.sh](/tmp/airtrust-observability-dr-readiness/scripts/deploy-worker-safe.sh:1);
- pipeline e guardas descritos em [DEPLOYMENT_AND_DEVOPS.md](/tmp/airtrust-observability-dr-readiness/DEPLOYMENT_AND_DEVOPS.md:1);
...
- script read-only de inventario: [scripts/audit-observability-dr-readiness.sh](/tmp/airtrust-observability-dr-readiness/scripts/audit-observability-dr-readiness.sh:1);
- runbook de restore drill seguro: [docs/operational-hardening/AIRTRUST_RESTORE_DRILL_RUNBOOK_20260621.md](/tmp/airtrust-observability-dr-readiness/docs/operational-hardening/AIRTRUST_RESTORE_DRILL_RUNBOOK_20260621.md:1);
- runbook de rollback de aplicacao: [docs/operational-hardening/AIRTRUST_ROLLBACK_RUNBOOK_20260621.md](/tmp/airtrust-observability-dr-readiness/docs/operational-hardening/AIRTRUST_ROLLBACK_RUNBOOK_20260621.md:1).
