...
export function getTypeMeta(type: TipoConteudo | null | undefined): {
  ...
  if (type === 'video') {
    return {
      ...
      chipClass: 'border border-fuchsia-200 bg-fuchsia-50 text-fuchsia-700',
...
export function getMatriculaStatusMeta(
  ...
  switch (status) {
    case 'EM_ANDAMENTO':
      return { label: 'Em andamento', className: 'bg-amber-100 text-amber-800' };
    case 'CONCLUIDO':
      return { label: 'Concluído', className: 'bg-emerald-100 text-emerald-800' };
    case 'REPROVADO':
      return { label: 'Reprovado', className: 'bg-rose-100 text-rose-800' };
    case 'CANCELADO':
      return { label: 'Cancelado', className: 'bg-slate-200 text-slate-700' };
    case 'RASCUNHO':
      return { label: 'Rascunho', className: 'bg-slate-200 text-slate-700' };
    case 'PUBLICADO':
      return { label: 'Publicado', className: 'bg-emerald-100 text-emerald-800' };
    case 'DISPONIVEL':
      return { label: 'Disponível', className: 'bg-sky-100 text-sky-800' };
    default:
      return { label: 'Não iniciado', className: 'bg-slate-100 text-slate-700' };
  }
}
...
export function LmsEmptyState({
  ...
  return (
    <div className="flex min-h-[220px] flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white px-6 py-10 text-center shadow-sm">
