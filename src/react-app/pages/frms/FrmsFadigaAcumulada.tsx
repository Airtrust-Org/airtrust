 ...
 * com barras de progresso coloridas (verde/amarelo/vermelho).
 */
...
function ProgressBar({
  ...
  return (
    <div className="relative w-full h-3 bg-slate-200 rounded-full overflow-hidden">
      ...
      />
      {/* Threshold markers */}
      <div className="absolute top-0 left-[80%] w-px h-full bg-emerald-600/40" />
      <div className="absolute top-0 left-[90%] w-px h-full bg-amber-600/40" />
      <div className="absolute top-0 left-[95%] w-px h-full bg-red-600/40" />
    </div>
  );
}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        {resumo && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <p className="text-2xl font-bold text-slate-900 mt-1">{resumo.total_tripulantes}</p>
            </div>
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 shadow-sm">
              <div className="flex items-center gap-2 text-amber-700 text-sm">
                <AlertTriangle className="h-4 w-4" /> Em alerta (≥80%)
              </div>
              <p className="text-2xl font-bold text-amber-700 mt-1">{resumo.em_alerta}</p>
            </div>
            <div className="rounded-xl border border-red-200 bg-red-50 p-4 shadow-sm">
              <div className="flex items-center gap-2 text-red-700 text-sm">
                <AlertTriangle className="h-4 w-4" /> Críticos (≥95%)
              </div>
              <p className="text-2xl font-bold text-red-700 mt-1">{resumo.criticos}</p>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500">
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 rounded-full bg-blue-400" /> &lt;80% Normal
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 rounded-full bg-emerald-500" /> ≥80% Atenção
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 rounded-full bg-amber-400" /> ≥90% Alerta
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 rounded-full bg-red-500" /> ≥95% Crítico
          </div>
        </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-slate-900 truncate">{t.nome}</span>
                        {t.funcao && (
                          <span className="text-xs px-1.5 py-0.5 rounded bg-slate-200/80 text-slate-600 whitespace-nowrap">
                            {t.funcao}
                          </span>
                        )}
                        {t.dia_ciclo && (
                          <span className="text-xs text-slate-500">Dia {t.dia_ciclo}</span>
                        )}
                      </div>
                      {/* Jornada bar */}
                ...
                {expandedTrip === t.tripulante_id && evolucao.length > 0 && (
                  <div className="mt-1 ml-4 mr-4 rounded-lg border border-slate-200 bg-white overflow-x-auto">
                    <table className="w-full min-w-[920px] text-xs">
                      ...
                      <tbody>
                        {evolucao.map((e) => (
                          <tr key={e.data} className="border-t border-slate-100 hover:bg-slate-50">
