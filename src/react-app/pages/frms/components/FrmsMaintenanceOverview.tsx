...
interface Props {
  ...
  counts: Record<MaintenanceDecisionBucket, number>;
  isLoading: boolean;
  error: unknown;
}

export default function FrmsMaintenanceOverview({ items, counts, isLoading, error }: Props) {
    ...
    <>
      ...
      {error ? (
        <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800 dark:border-red-900/60 dark:bg-red-950/30 dark:text-red-200">
          {error instanceof Error ? error.message : 'Não foi possível carregar a equipe de manutenção.'}
        </div>
      ) : null}
                  ...
                  <tr key={item.funcionario_id} className="align-top hover:bg-slate-50/70 dark:hover:bg-slate-900/40">
                    <td className="px-4 py-3">
                      ...
                      <p className="mt-0.5 text-xs text-slate-500">
                        {cargoLabel(item)} · {item.setor_nome || 'Manutenção'}
                      </p>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className="font-medium text-slate-800 dark:text-slate-100">
                        {readinessLabel(item.readiness_classification)}
                      </span>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <div className="flex gap-3 rounded-xl border border-slate-200 p-4 dark:border-slate-800">
          ...
          <div>
            <p className="text-sm font-bold text-slate-900 dark:text-white">Revisão imediata</p>
            <p className="text-xs text-slate-500">Não é decisão médica automática; exige avaliação operacional antes de tarefa crítica.</p>
          </div>
        </div>
        <div className="flex gap-3 rounded-xl border border-slate-200 p-4 dark:border-slate-800">
          <AlertTriangle className="h-5 w-5 flex-none text-orange-500" />
          <div>
            <p className="text-sm font-bold text-slate-900 dark:text-white">Atenção</p>
            <p className="text-xs text-slate-500">Considere pausa, redistribuição, supervisão ou inspeção independente conforme a tarefa.</p>
          </div>
        </div>
        <div className="flex gap-3 rounded-xl border border-slate-200 p-4 dark:border-slate-800">
          <Clock3 className="h-5 w-5 flex-none text-amber-500" />
          <div>
            <p className="text-sm font-bold text-slate-900 dark:text-white">Pendente</p>
            <p className="text-xs text-slate-500">Ausência de dado não é tratada como condição normal.</p>
          </div>
        </div>
      </div>
    </>
