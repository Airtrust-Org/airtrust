...
function statusToLabel(status: string, hasData: boolean, teveJornada: boolean): string {
  ...
  if (teveJornada)             return 'Jornada registrada';
  return 'Sem jornada confirmada';
}
  ...
  return (
    <div className="min-w-[160px] rounded-xl border border-slate-200 bg-white p-3 text-xs shadow-lg">
      ...
      {d.teveJornada ? (
        ...
        <p className="text-slate-400">Sem jornada confirmada neste dia</p>
      ) : (
        <p className="text-slate-400">Fonte não classificada — requer confirmação manual.</p>
      )}
      ...
      <div className="flex flex-wrap gap-2 text-[11px]">
        ...
        {canonicalAccum == null && timeline?.summary.cumulative_duty_min != null && (
          <span className="rounded-md border border-amber-200 bg-amber-50 px-2 py-0.5 font-medium text-amber-800">
            Acumulado estimado: {formatFortnightMinutes(timeline.summary.cumulative_duty_min)}
          </span>
        )}
        {jornadaDays > 0 && (
        ...
        <>
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-slate-600">
            ...
            <span className="inline-flex items-center gap-1.5 italic text-slate-400">
              Dias sem barra = sem jornada confirmada ou fonte não classificada
            </span>
          </div>
        </>
      )}
