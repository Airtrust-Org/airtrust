...
const WORKFLOW_COLORS: Record<CvRdv['workflow_status'], string> = {
  ...
  reaberto: 'bg-violet-100 text-violet-800 dark:bg-violet-900/30 dark:text-violet-300',
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-900">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span
          className={`rounded-full px-3 py-1 text-xs font-semibold ${WORKFLOW_COLORS[rdv.workflow_status]}`}
        >
      ...
      {rdv.workflow_status === 'reaberto' && (
        <div className="mb-4 rounded-lg border border-violet-200 bg-violet-50 p-3 text-xs text-violet-800 dark:border-violet-800 dark:bg-violet-950/20 dark:text-violet-300">
          <strong>RDV reaberto:</strong> aguardando nova revisão da Coordenação.
        </div>
      )}
      ...
      <div className="space-y-2">
        {!isCoordenacao &&
          ...
          rdv.status === 'preenchimento_finalizado' && (
            <ActionButton
              ...
              onClick={() =>
                runAction(() => enviar.mutateAsync({ vooId, body: { versao: rdv.versao } }))
              }
            >
              {bloqueiosEnvio.length > 0
                ? 'Envio bloqueado por alertas'
                : 'Enviar para Coordenação'}
            </ActionButton>
          )}

        {isCoordenacao &&
          (rdv.workflow_status === 'enviado' || rdv.workflow_status === 'reaberto') && (
            <ActionButton
              icon={Eye}
              loading={iniciarRevisao.isPending}
              onClick={() =>
                runAction(() => iniciarRevisao.mutateAsync({ vooId, body: { versao: rdv.versao } }))
              }
            >
              Iniciar revisão
            </ActionButton>
        ...
        {isCoordenacao && rdv.workflow_status === 'em_revisao' && (
          <>
            <ActionButton
              ...
              onClick={() =>
                runAction(() => aprovar.mutateAsync({ vooId, body: { versao: rdv.versao } }))
              }
            >
            ...
            {prompt === 'devolver' ? (
              <JustificativaPrompt
                ...
                onConfirm={(justificativa) =>
                  runAction(() =>
                    devolver.mutateAsync({ vooId, body: { versao: rdv.versao, justificativa } }),
                  )
                }
              />
            ) : (
        ...
        {isCoordenacao && rdv.workflow_status === 'aprovado_coordenacao' && (
          <ActionButton
            ...
            onClick={() =>
              runAction(() => finalizar.mutateAsync({ vooId, body: { versao: rdv.versao } }))
            }
          >
            Finalizar relatório
          </ActionButton>
          ...
          <>
            {prompt === 'reabrir' ? (
              <JustificativaPrompt
                ...
                onConfirm={(justificativa) =>
                  runAction(() =>
                    reabrir.mutateAsync({ vooId, body: { versao: rdv.versao, justificativa } }),
                  )
                }
              />
            ) : (
        ...
        {isCoordenacao &&
          ['rascunho', 'enviado', 'em_revisao', 'devolvido', 'reaberto'].includes(
            rdv.workflow_status,
          ) && (
            <>
              {prompt === 'cancelar' ? (
                <JustificativaPrompt
                  ...
                  onConfirm={(justificativa) =>
                    runAction(() =>
                      cancelar.mutateAsync({ vooId, body: { versao: rdv.versao, justificativa } }),
                    )
                  }
                />
              ) : (
      ...
      >
        ...
        {showHistorico ? 'Ocultar histórico' : `Ver histórico (v${rdv.versao})`}
      </button>
              ...
              <ul className="space-y-1 text-xs text-slate-600 dark:text-slate-300">
                {revisoes.map((r) => (
                  <li key={r.id}>
                    ...
                    <span className="line-through">{r.valor_anterior ?? '—'}</span> →{' '}
