...
import { Plane, FileText, AlertTriangle, Users, Wrench, CheckCircle, TrendingUp } from 'lucide-react';
...
export default function ControleVoosDashboard() {
  ...
  ].filter(Boolean) as { id: string; mensagem: string; gravidade: 'critico' | 'alerta' | 'info' }[] : [];
        ...
        <ControleVoosPageShell>
          ...
          {error && (
            <div className="rounded-xl border border-red-200 bg-red-50 p-6 dark:border-red-800 dark:bg-red-950/20">
              <p className="text-sm text-red-700 dark:text-red-300">Erro ao carregar dashboard: {error.message}</p>
            </div>
          )}
            ...
            <>
              {dashboard && (
                <p className="mb-4 text-xs text-slate-400 dark:text-slate-500">
                  ...
                  {dashboard.periodo.data_inicio !== dashboard.periodo.data_fim ? ` — ${formatDate(dashboard.periodo.data_fim)}` : ''}
                </p>
              )}
              ...
              <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  { to: `/controle-voos/jornadas?data=${selectedDate}`, icon: <Users className="h-5 w-5 text-teal-500" />, label: 'Jornadas' },
              ...
              {alertasOperacionais.length === 0 && !isLoading && totais && (
                <div className="mb-8 rounded-xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-800 dark:bg-emerald-950/20">
                  <p className="text-sm text-emerald-700 dark:text-emerald-300">Nenhum alerta operacional no período.</p>
                </div>
              )}
              ...
              {proximosVoos.length > 0 && (
                <section className="mb-8">
                  <h2 className="mb-3 text-lg font-semibold text-slate-800 dark:text-slate-100">Próximos voos do período</h2>
              ...
              {proximosVoos.length === 0 && !isLoading && totais && (
                <div className="mb-8 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center dark:border-slate-700 dark:bg-slate-900">
                  ...
                  <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">Nenhum voo encontrado no período.</p>
                  <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">Crie o primeiro voo na tela de Programação.</p>
                </div>
              )}
            </>
          )}
