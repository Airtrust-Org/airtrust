...
# Script para atualizar domain airtrust.online para branch production no Pages
...
# Primeiro, remover domain para fazer clean
...
if echo "$REMOVE_RESP" | grep -q '"success":true'; then
  ...
  echo "Response: $(echo $REMOVE_RESP | python3 -m json.tool 2>/dev/null || echo $REMOVE_RESP)"
fi

sleep 2

# Adicionar domain novamente com branch específico
...
if echo "$ADD_RESP" | grep -q '"success":true'; then
  ...
  echo ""
  echo "🔍 Próximo passo: Execute ./verificar-versao-producao.sh"
else
  echo ""
  echo "❌ Erro ao adicionar domain"
fi
