...
const workflow = readFileSync(
  resolve(root, '.github/workflows/staging-0438-legacy-validation.yml'),
  'utf8',
);
const physical = readFileSync(
  resolve(root, 'scripts/staging/validate-0438-legacy-physical-state.sh'),
  'utf8',
);
const tenant = readFileSync(
  resolve(root, 'scripts/staging/validate-0438-e2e-tenant-records.mjs'),
  'utf8',
);

const forbiddenRemoteMutation =
  /\b(?:INSERT\s+INTO|UPDATE\s+[A-Za-z_]|DELETE\s+FROM|ALTER\s+TABLE|DROP\s+TABLE|CREATE\s+TABLE)\b/i;
...
test('E2E CAS contract enforces version on corrigir_apos_devolucao and bumps rdvVersao', () => {
  const e2eScript = readFileSync(
    resolve(root, 'scripts/staging/run-controle-voos-e2e.mjs'),
    'utf8',
  );

  // Verify operation corrigir_apos_devolucao sends versao: rdvVersao
  const match = e2eScript.match(
    /operation:\s*'corrigir_apos_devolucao'[\s\S]*?body:\s*\{([\s\S]*?)\}/,
  );
  assert.ok(match, 'corrigir_apos_devolucao operation must exist');
  assert.match(
    match[1],
    /versao:\s*rdvVersao/,
    'corrigir_apos_devolucao body must include versao: rdvVersao',
  );
  ...
  assert.match(
    afterCall,
    /rdvVersao\s*\+=\s*1/,
    'rdvVersao must be incremented after corrigir_apos_devolucao',
  );
...
test('fixture cleanup script is fail closed, handles FKs child-first, and never touches production', () => {
  const cleanupScript = readFileSync(
    resolve(root, 'scripts/staging/cleanup-controle-voos-e2e-fixtures.mjs'),
    'utf8',
  );
  ...
  assert.doesNotMatch(
    cleanupScript,
    /AVISO: comando falhou \(continuando cleanup\)/,
    'D1 errors must not be swallowed',
  );
  assert.match(
    cleanupScript,
    /throw new Error\(`D1 execute falhou \[/,
    'failed D1 execute must throw with statement label',
  );
  ...
  assert.ok(
    deleteLoopIdx > 0 && verifyIdx > deleteLoopIdx && rmManifestIdx > verifyIdx,
    'manifest removal must happen strictly after delete loop and verification',
  );

  // Safety & FK governance
  assert.doesNotMatch(
    cleanupScript,
    /foreign_keys\s*=\s*OFF/i,
    'PRAGMA foreign_keys=OFF is forbidden',
  );
  ...
  assert.ok(
    refreshTokensIdx < usuariosIdx,
    'refresh_tokens must be deleted before usuarios to avoid FK error',
  );
  assert.ok(usuariosEmpresasIdx < usuariosIdx, 'usuarios_empresas must be deleted before usuarios');
  assert.ok(
    usuariosIdx < funcionariosIdx,
    'usuarios must be deleted before funcionarios because usuarios.funcionario_id references funcionarios',
  );
  assert.ok(
    funcionariosIdx < setoresIdx,
    'funcionarios must be deleted before setores because funcionarios.setor_id references setores',
  );
...
test('cleanup script removes manifest only when all delete statements and verification succeed', () => {
  ...
  try {
    ...
    assert.ok(
      ...
      'manifest file must be removed when all deletes and verification pass',
    );
  } finally {
    rmSync(tmp, { recursive: true, force: true });
  }
