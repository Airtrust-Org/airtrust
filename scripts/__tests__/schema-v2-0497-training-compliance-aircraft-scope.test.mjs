...
test('official Schema V2 builder accepts 0497 and appends one ledger row', () => {
  ...
  const result = buildReviewedSchemaApply({ manifestPath: MANIFEST, outputPath, expectedChangeId: CHANGE_ID, githubSha: 'eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee' });
...
test('production workflow wires dedicated 0497 preflight and postconditions', () => {
  ...
  for (const file of ['scripts/schema-v2/validate-0497-production-preflight.sh','scripts/schema-v2/validate-0497-production-postconditions.sh']) {
...
test('staging allowlists 0497 and routes it through its guarded runner', () => {
  ...
  const runner = readFileSync('scripts/staging/apply-0497-training-compliance-aircraft-scope.sh', 'utf8');
