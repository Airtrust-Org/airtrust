...
TMP_DIR="$(mktemp -d)"
PPTX_FILE="$TMP_DIR/lms-demo.pptx"
...
node --input-type=module - "$PPTX_FILE" <<'NODE'
...
<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types">
  ...
  'ppt/presentation.xml': strToU8(`<?xml version="1.0" encoding="UTF-8"?>
...
<p:sld xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main">
  <p:cSld>
    <p:spTree>
      <p:sp>
        <p:txBody>
          <a:p><a:r><a:t>Briefing operacional</a:t></a:r></a:p>
          <a:p><a:r><a:t>Validar leitura completa do fluxo LMS em formato PPTX.</a:t></a:r></a:p>
        </p:txBody>
      </p:sp>
    </p:spTree>
  </p:cSld>
</p:sld>`),
  'ppt/slides/slide2.xml': strToU8(`<?xml version="1.0" encoding="UTF-8"?>
<p:sld xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main">
  <p:cSld>
    <p:spTree>
      <p:sp>
        <p:txBody>
          <a:p><a:r><a:t>Critério de conclusão</a:t></a:r></a:p>
          <a:p><a:r><a:t>O colaborador precisa percorrer todos os slides antes da conclusão.</a:t></a:r></a:p>
        </p:txBody>
      </p:sp>
    </p:spTree>
  </p:cSld>
...
fs.writeFileSync(outputPath, Buffer.from(zipSync(files, { level: 0 })));
NODE
...
CURSO_ID="$(sql "SELECT id FROM lms_cursos WHERE empresa_id = $EMPRESA_ID AND deleted_at IS NULL AND qualificacao_tipo_id = $QUALIFICACAO_ID AND tipo_conteudo = 'pptx' ORDER BY id LIMIT 1;")"
...
CURSO_TITULO="${AIRTRUST_LMS_CURSO_TITULO:-$QUALIFICACAO_NOME · PPTX}"
...
COURSE_PAYLOAD="$(node -e '
  ...
  process.stdout.write(JSON.stringify({
    titulo: title,
    descricao: "Seed local para validar o player PPTX do LMS ponta a ponta.",
    categoria: "EAD",
    carga_horaria_minutos: 20,
    conteudo_programatico: "Leitura sequencial de slides operacionais com bloqueio de conclusão antecipada.",
    observacoes: "Curso seed gerado automaticamente para validacao local do player PPTX.",
    carga_horaria_inicial_horas: 0.5,
    carga_horaria_recorrente_horas: 0.5,
...
UPLOAD_RESPONSE="$(curl -fsS -X POST "$API_BASE/lms/cursos/$CURSO_ID/upload/pptx" \
  -H "Authorization: Bearer $TOKEN" \
  -F "arquivo=@$PPTX_FILE;type=application/vnd.openxmlformats-officedocument.presentationml.presentation")"
SLIDE_COUNT="$(printf '%s' "$UPLOAD_RESPONSE" | json_get data.slide_count)"
...
if [[ -z "$MATRICULA_ID" ]]; then
  MATRICULA_RESPONSE="$(curl -fsS -X POST "$API_BASE/lms/matriculas" \
    -H "Authorization: Bearer $TOKEN" \
    -H 'Content-Type: application/json' \
    -d "{\"curso_id\":$CURSO_ID,\"funcionario_id\":$FUNCIONARIO_ID,\"observacoes\":\"Seed local LMS PPTX\"}")"
...
echo "PLAYER_URL=http://localhost:3000/lms/player/pptx/$MATRICULA_ID"