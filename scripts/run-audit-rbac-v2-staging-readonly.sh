...
fail() {
  echo "AUDIT_RBAC_V2_STAGING_READONLY=FAIL"
  echo "REASON=$1"
  exit 1
}
...
user_platform_roles_exists="$(query_remote_field "SELECT COUNT(*) AS total FROM sqlite_master WHERE type='table' AND name='user_platform_roles';" total)"
support_access_grants_exists="$(query_remote_field "SELECT COUNT(*) AS total FROM sqlite_master WHERE type='table' AND name='support_access_grants';" total)"
support_access_sessions_exists="$(query_remote_field "SELECT COUNT(*) AS total FROM sqlite_master WHERE type='table' AND name='support_access_sessions';" total)"
audit_events_v2_exists="$(query_remote_field "SELECT COUNT(*) AS total FROM sqlite_master WHERE type='table' AND name='audit_events_v2';" total)"
objects_0389_count="$(
  query_remote_field \
...
d1_migrations_rows="$(query_remote_field "SELECT COUNT(*) AS total FROM d1_migrations;" total)"
ledger_0385_rows="$(query_remote_field "SELECT COUNT(*) AS total FROM d1_migrations WHERE name = '0385_audit_events_v2.sql';" total)"
ledger_0389_rows="$(query_remote_field "SELECT COUNT(*) AS total FROM d1_migrations WHERE name = '0389_platform_roles_support_access_foundation.sql';" total)"
user_platform_roles_rows="$(table_row_count_or_zero "$user_platform_roles_exists" "user_platform_roles")"
support_access_grants_rows="$(table_row_count_or_zero "$support_access_grants_exists" "support_access_grants")"
support_access_sessions_rows="$(table_row_count_or_zero "$support_access_sessions_exists" "support_access_sessions")"
audit_events_v2_rows="$(table_row_count_or_zero "$audit_events_v2_exists" "audit_events_v2")"

echo "AUDIT_RBAC_V2_STAGING_READONLY=PASS"
echo "TARGET=staging"
...
echo "OBJECTS_0385_COUNT=$objects_0385_count"
echo "D1_MIGRATIONS_ROWS=$d1_migrations_rows"
echo "LEDGER_0385_ROWS=$ledger_0385_rows"
echo "LEDGER_0389_ROWS=$ledger_0389_rows"
...
echo "AUDIT_EVENTS_V2_ROWS=$audit_events_v2_rows"
