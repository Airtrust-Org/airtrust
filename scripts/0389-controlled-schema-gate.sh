...
MIGRATION_FILE="$ROOT_DIR/worker-airtrust/migrations/0389_platform_roles_support_access_foundation.sql"
READONLY_SCRIPT="$ROOT_DIR/scripts/run-0389-staging-schema-readonly.sh"
APPLY_SCRIPT="$ROOT_DIR/scripts/run-0389-staging-schema-apply.sh"

fail() {
  echo "CONTROLLED_0389_SCHEMA_GATE=BLOCKED"
  echo "REASON=$1"
  exit 2
}
...
[[ "$mode" == "0389-schema" ]] || fail "mode_must_be_0389_schema"
...
printf '%s' "$safe_command" | LC_ALL=C grep -Eiq '0389' || fail "safe_command_missing_0389_marker"

echo "CONTROLLED_0389_SCHEMA_GATE=READY_FOR_MANUAL_CONTROLLED_EXECUTION"
echo "MODE=0389-schema"
echo "TARGET=staging"
...
echo "NOTE=0389 schema package is constrained to staging, approved remote D1 access and the dedicated apply wrapper."
