...
MIGRATION_BASENAME="0504_controle_voos_operational_model.sql"
SCHEMA_CHANGE_ID="controle-voos-operational-model-0504"
...
[[ "$migration_arg" == "$expected_path" ]] || { echo "ERROR: 0504 requires exact path $expected_path" >&2; exit 1; }
[[ ! -L "$migration_arg" && -f "$migration_arg" ]] || { echo "ERROR: migration 0504 missing or symlink refused" >&2; exit 1; }
if ! git -C release diff --quiet -- "worker-airtrust/migrations/$MIGRATION_BASENAME" ||    ! git -C release diff --cached --quiet -- "worker-airtrust/migrations/$MIGRATION_BASENAME"; then
  echo "ERROR: migration 0504 has uncommitted release changes" >&2
  exit 1
fi
...
plan_path="release/worker-airtrust/schema-v2/plans/controle-voos-operational-model-0504.md"
for path in "$manifest_path" "$schema_sql_path" "$plan_path"; do
  [[ ! -L "$path" && -f "$path" ]] || { echo "ERROR: reviewed Schema V2 artifact missing: $path" >&2; exit 1; }
done
...
if (
  manifest.changeId !== 'controle-voos-operational-model-0504' ||
  manifest.baselineId !== 'production-d1-baseline-v2-20260714' ||
  manifest.filePath !== 'worker-airtrust/schema-v2/changes/0504_controle_voos_operational_model.sql' ||
  manifest.planPath !== 'worker-airtrust/schema-v2/plans/controle-voos-operational-model-0504.md' ||
...
const result=spawnSync('npx',['wrangler','d1','execute',db,'--remote','--json','--command',sql],{cwd:path.join(process.cwd(),'worker-airtrust'),encoding:'utf8',env:process.env});
...
for table in cv_voos cv_voo_tripulantes cv_voo_abastecimentos cv_tipos_voo cv_naturezas_voo cv_aeroportos d1_migrations; do
...
table_count="$(query_count "SELECT COUNT(*) count FROM sqlite_master WHERE type='table' AND name IN ('cv_contratos','cv_funcoes_bordo');")"
if [[ "$ledger_count" == 1 ]]; then
  bash scripts/staging/validate-0504-postconditions.sh --target="$db_name"
  echo "MIGRATION_ALREADY_APPLIED_AND_VALIDATED=$MIGRATION_BASENAME"
  exit 0
fi
[[ "$ledger_count" == 0 && "$table_count" == 0 ]] || { echo "ERROR: 0504 schema/ledger drift or partial apply" >&2; exit 1; }
