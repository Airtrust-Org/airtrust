...
assert_count "cpf-index-contract" "1" "SELECT COUNT(*) AS count FROM sqlite_master WHERE type='index' AND name='ux_funcionarios_cpf_empresa_active' AND LOWER(REPLACE(REPLACE(REPLACE(REPLACE(COALESCE(sql,''),' ',''),char(9),''),char(10),''),char(13),'')) = 'createuniqueindexux_funcionarios_cpf_empresa_activeonfuncionarios(empresa_id,cpf)wheredeleted_atisnullandcpfisnotnullandtrim(cpf)!='''';"
assert_count "matricula-index-contract" "1" "SELECT COUNT(*) AS count FROM sqlite_master WHERE type='index' AND name='ux_funcionarios_matricula_empresa_active' AND LOWER(REPLACE(REPLACE(REPLACE(REPLACE(COALESCE(sql,''),' ',''),char(9),''),char(10),''),char(13),'')) = 'createuniqueindexux_funcionarios_matricula_empresa_activeonfuncionarios(empresa_id,trim(matricula))wheredeleted_atisnullandmatriculaisnotnullandtrim(matricula)!='''';"
assert_count "email-index-contract" "1" "SELECT COUNT(*) AS count FROM sqlite_master WHERE type='index' AND name='ux_funcionarios_email_empresa_active' AND LOWER(REPLACE(REPLACE(REPLACE(REPLACE(COALESCE(sql,''),' ',''),char(9),''),char(10),''),char(13),'')) = 'createuniqueindexux_funcionarios_email_empresa_activeonfuncionarios(empresa_id,lower(trim(email)))wheredeleted_atisnullandemailisnotnullandtrim(email)!='''';"
