...
[[ "$migration_arg" == "$expected_path" ]] || {
  echo "ERROR: 0501 requires exact path $expected_path" >&2
  exit 1
}
[[ ! -L "$migration_arg" && -f "$migration_arg" ]] || {
  echo "ERROR: migration 0501 missing or symlink refused" >&2
  exit 1
}
if ! git -C release diff --quiet -- "worker-airtrust/migrations/$MIGRATION_BASENAME" || \
  ...
  echo "ERROR: migration 0501 has uncommitted release changes" >&2
  exit 1
fi
...
plan_path="release/worker-airtrust/schema-v2/plans/controle-voos-leg-operational-weights-0501.md"
...
if (
  manifest.changeId !== 'controle-voos-leg-operational-weights-0501' ||
...
for table in empresas aeronaves cv_voo_etapas cv_naturezas_voo d1_migrations; do
...
[[ "$(query_count "SELECT COUNT(*) count FROM empresas WHERE id=6 AND deleted_at IS NULL;")" == 1 ]] || {
  echo "ERROR: tenant 6 prerequisite missing in staging" >&2
  exit 1
}

ledger_count="$(query_count "SELECT COUNT(*) count FROM d1_migrations WHERE name='$MIGRATION_BASENAME';")"
aircraft_column_count="$(query_count "SELECT COUNT(*) count FROM pragma_table_info('aeronaves') WHERE name IN ('peso_vazio','unidade_peso');")"
stage_column_count="$(query_count "SELECT COUNT(*) count FROM pragma_table_info('cv_voo_etapas') WHERE name IN ('peso_passageiros','peso_bagagem','peso_tripulacao','peso_vazio','peso_total','unidade_peso','observacoes');")"
...
[[ "$ledger_count" == 0 && "$aircraft_column_count" == 0 && "$stage_column_count" == 0 ]] || {
  echo "ERROR: 0501 schema/ledger drift or partial apply" >&2
  exit 1
}
