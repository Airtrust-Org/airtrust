...
if [[ "$migration_basename" == "0488_controle_voos_pilot_offline_sync_receipts.sql" ]]; then
  args=(--migration="$migration_path")
  $apply && args+=(--apply)
  exec bash "$ROOT/scripts/staging/apply-0488-pilot-offline-sync-receipts.sh" "${args[@]}"
fi

if [[ "$migration_basename" == "0491_training_compliance_requirements.sql" ]]; then
  args=(--migration="$migration_path")
  $apply && args+=(--apply)
  exec bash "$ROOT/scripts/staging/apply-0491-training-compliance-requirements.sh" "${args[@]}"
fi

if [[ "$migration_basename" == "0492_organizational_structure_normalization.sql" ]]; then
  args=(--migration="$migration_path")
  $apply && args+=(--apply)
  exec bash "$ROOT/scripts/staging/apply-0492-organizational-structure-normalization.sh" "${args[@]}"
fi

if [[ "$migration_basename" == "0494_training_enrollment_reconciliation.sql" ]]; then
  args=(--migration="$migration_path")
  $apply && args+=(--apply)
  exec bash "$ROOT/scripts/staging/apply-0494-training-enrollment-reconciliation.sh" "${args[@]}"
fi

if [[ "$migration_basename" == "0497_training_compliance_aircraft_scope.sql" ]]; then
  args=(--migration="$migration_path")
  $apply && args+=(--apply)
  exec bash "$ROOT/scripts/staging/apply-0497-training-compliance-aircraft-scope.sh" "${args[@]}"
fi

if [[ "$migration_basename" == "0498_frms_parametric_v2.sql" ]]; then
  args=(--migration="$migration_path")
  $apply && args+=(--apply)
  exec bash "$ROOT/scripts/staging/apply-0498-frms-parametric-v2.sh" "${args[@]}"
fi

if [[ "$migration_basename" == "0499_frms_v2_historical_backfill.sql" ]]; then
  args=(--migration="$migration_path")
  $apply && args+=(--apply)
  exec bash "$ROOT/scripts/staging/apply-0499-frms-v2-historical-backfill.sh" "${args[@]}"
fi

if [[ "$migration_basename" == "0500_controle_voos_navigation_points.sql" ]]; then
  args=(--migration="$migration_path")
  $apply && args+=(--apply)
  exec bash "$ROOT/scripts/staging/apply-0500-controle-voos-navigation-points.sh" "${args[@]}"
fi

if [[ "$migration_basename" == "0501_controle_voos_leg_operational_weights.sql" ]]; then
  args=(--migration="$migration_path")
  $apply && args+=(--apply)
  exec bash "$ROOT/scripts/staging/apply-0501-controle-voos-leg-operational-weights.sh" "${args[@]}"
fi

if [[ "$migration_basename" == "0502_controle_voos_fueling_companies.sql" ]]; then
  args=(--migration="$migration_path")
  $apply && args+=(--apply)
  exec bash "$ROOT/scripts/staging/apply-0502-controle-voos-fueling-companies.sh" "${args[@]}"
fi

if [[ "$migration_basename" == "0503_conhecimento_ativo_foundation.sql" ]]; then
  args=(--migration="$migration_path")
  $apply && args+=(--apply)
  exec bash "$ROOT/scripts/staging/apply-0503-conhecimento-ativo-foundation.sh" "${args[@]}"
fi

if [[ "$migration_basename" == "0504_controle_voos_operational_model.sql" ]]; then
  args=(--migration="$migration_path")
  $apply && args+=(--apply)
  exec bash "$ROOT/scripts/staging/apply-0504-controle-voos-operational-model.sh" "${args[@]}"
fi
...
if [[ "$migration_basename" == "0424_examiner_universal_training_fichas.sql" || \
      ...
      "$migration_basename" == "0507_controle_voos_delay_justification_catalog.sql" || \
      "$migration_basename" == "0508_training_compliance_daily_snapshots.sql" || \
      "$migration_basename" == "0509_controle_voos_flight_plan.sql" || \
      "$migration_basename" == "0510_controle_voos_petrobras_rve_export.sql" || \
      "$migration_basename" == "0511_frms_duty_boundary_config.sql" ]]; then
  args=(--migration="$migration_path")
  $apply && args+=(--apply)
