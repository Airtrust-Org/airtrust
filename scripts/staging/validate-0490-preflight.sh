#!/usr/bin/env bash
# Read-only fail-closed staging preflight for migration 0490.
set -euo pipefail
umask 077
...
[[ "$target" == "$ALLOWED_DB_NAME" ]] || { echo "ERROR: 0490 staging preflight refused target: $target" >&2; exit 1; }
...
assert_count "unapplied-migration" 0 "SELECT COUNT(*) AS count FROM d1_migrations WHERE name='$MIGRATION_NAME';"
...
echo "SIMULATOR_PLANNING_CURRICULUM_0490_STAGING_PREFLIGHT=PASS"
