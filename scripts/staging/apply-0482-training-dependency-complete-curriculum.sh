...
# operational_decision: apply the reviewed additive 0482 complete-curriculum enrichment after 0481 and before final staging regression.
# dry_run_required: default mode performs target/path/schema/ledger preflight only; --apply is explicit.
...
if [[ "$prerequisite_count" != "1" ]]; then
  echo "ERROR: 0482 exige 0481 aplicada exatamente uma vez no ledger; encontrado=$prerequisite_count." >&2
  exit 1
fi
echo "PREREQUISITE_LEDGER_CONFIRMED=$PREREQUISITE_MIGRATION"
...
writeFileSync(outputPath, buildLedgerAppliedSql({ migrationSql, migrationName }), {
  encoding: 'utf8',
  mode: 0o600,
});
NODE
