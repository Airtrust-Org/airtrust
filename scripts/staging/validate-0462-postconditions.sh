...
test "$(run "SELECT COUNT(*) n FROM sqlite_master WHERE type='index' AND name='idx_qualificacoes_tipos_codigo_empresa_active' AND upper(sql) LIKE '%UNIQUE INDEX%ON QUALIFICACOES_TIPOS(EMPRESA_ID, CODIGO COLLATE NOCASE)%WHERE DELETED_AT IS NULL%'" | node -e 'let d="";process.stdin.on("data",c=>d+=c).on("end",()=>process.stdout.write(String(JSON.parse(d)[0]?.n??0)))')" = 1
