...
SCHEMA_CHANGE_ID="controle-voos-fueling-companies-0502"
...
[[ "$migration_arg" == "$expected_path" ]] || { echo "ERROR: 0502 requires exact path $expected_path" >&2; exit 1; }
[[ ! -L "$migration_arg" && -f "$migration_arg" ]] || { echo "ERROR: migration 0502 missing or symlink refused" >&2; exit 1; }
...
plan_path="release/worker-airtrust/schema-v2/plans/controle-voos-fueling-companies-0502.md"
...
if (
  manifest.changeId !== 'controle-voos-fueling-companies-0502' ||
...
for table in cv_voo_abastecimentos d1_migrations; do
...
table_count="$(query_count "SELECT COUNT(*) count FROM sqlite_master WHERE type='table' AND name='cv_empresas_abastecimento';")"
...
[[ "$ledger_count" == 0 && "$table_count" == 0 ]] || { echo "ERROR: 0502 schema/ledger drift or partial apply" >&2; exit 1; }
