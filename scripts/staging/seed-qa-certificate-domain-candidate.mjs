...
// source_reference: issue #568 staging run 30703547716 proved that the QA
// tenant has no genuinely unclassified qualification type left to exercise
// the certificate fail-closed/classify/rollback path.
// operational_decision: STAGING_ONLY; provision one durable synthetic type and
// history by exact QA natural keys, with categoria_id/domain override NULL.
...
export function validateCandidateTarget({ dbName, dbId }) {
  ...
  if (id !== QA_CERT_CANDIDATE.allowedD1Id || id === QA_CERT_CANDIDATE.blockedProductionD1Id) {
...
export function buildCandidateSql(typeColumnsInput, historyColumnsInput, mode = 'apply') {
  ...
  const e = sqlString;
  const empresaId = `(SELECT id FROM empresas WHERE codigo = ${e(q.empresaCodigo)} AND ativo = 1 AND deleted_at IS NULL LIMIT 1)`;
  ...
  if (mode === 'rollback') {
    return `UPDATE qualificacoes_historico SET deleted_at = datetime('now'), updated_at = datetime('now')\nWHERE empresa_id = ${empresaId} AND observacoes = ${e(q.historyMarker)} AND deleted_at IS NULL;\n\nUPDATE qualificacoes_tipos SET deleted_at = datetime('now'), ativo = 0, updated_at = datetime('now')\nWHERE empresa_id = ${empresaId} AND codigo = ${e(q.tipoCodigo)} AND deleted_at IS NULL;`;
  }
  ...
  const typeValues = {
    ...
    conteudo_programatico: e('Validação sintética do fluxo de certificado por domínio operacional.'),
    validade: '12',
  ...
  const historyValues = {
    ...
    funcionario_cpf: `(SELECT cpf FROM funcionarios WHERE id = ${funcionarioId})`,
  ...
  return `
UPDATE funcionarios
SET cpf = COALESCE(NULLIF(TRIM(cpf), ''), ${e(q.syntheticCpf)}), updated_at = datetime('now')
WHERE id = ${funcionarioId};

UPDATE empresas_config
SET certificado_template_html = CASE
      WHEN TRIM(COALESCE(certificado_template_html, '')) = '' THEN ${e(TEMPLATE_HTML)}
      ELSE certificado_template_html
    END,
    deleted_at = NULL,
    updated_at = datetime('now')
WHERE empresa_id = ${empresaId};

INSERT INTO empresas_config (empresa_id, certificado_template_html, timezone, idioma, created_at, updated_at, deleted_at)
SELECT ${empresaId}, ${e(TEMPLATE_HTML)}, 'America/Sao_Paulo', 'pt-BR', datetime('now'), datetime('now'), NULL
...
export function buildCandidatePostconditionSql() {
  ...
  return `SELECT
    ...
    (SELECT COUNT(*) FROM empresas_config ec INNER JOIN empresas emp ON emp.id = ec.empresa_id
      WHERE emp.codigo = ${e(q.empresaCodigo)} AND ec.deleted_at IS NULL
...
export async function main() {
  ...
  const typeColumns = columnNames(executeWranglerJson(target.dbName, ['--command', "PRAGMA table_info('qualificacoes_tipos')"]));
  const historyColumns = columnNames(executeWranglerJson(target.dbName, ['--command', "PRAGMA table_info('qualificacoes_historico')"]));
  const mode = rollback ? 'rollback' : apply ? 'apply' : 'dry-run';
  const reportPath = String(process.env.QA_CERT_CANDIDATE_REPORT_PATH || '').trim();
  const report = { success: false, mode, target: target.dbName, before: null, after: null, changed: false, error: null };

  try {
    const before = firstRow(executeWranglerJson(target.dbName, ['--command', buildCandidatePostconditionSql()]));
    report.before = before;
    if (!apply && !rollback) {
      report.success = true;
      report.after = before;
      if (reportPath) writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
      console.log(JSON.stringify({ success: true, mode, before }));
      return;
    }
    const expected = rollback ? QA_CERT_CANDIDATE.rollbackConfirmation : QA_CERT_CANDIDATE.applyConfirmation;
    if (process.env.CONFIRM_STAGING_QA_CERT_CANDIDATE !== expected) throw new Error(`QA_CERT_CONFIRMATION_REQUIRED:${expected}`);
    ...
    writeFileSync(sqlFile, buildCandidateSql([...typeColumns], [...historyColumns], rollback ? 'rollback' : 'apply'), { mode: 0o600 });
    try {
    ...
    const after = firstRow(executeWranglerJson(target.dbName, ['--command', buildCandidatePostconditionSql()]));
    report.after = after;
    const expectedCount = rollback ? 0 : 1;
    if (Number(after.type_count) !== expectedCount || Number(after.history_count) !== expectedCount) {
      throw new Error(`QA_CERT_POSTCONDITION:type=${after.type_count};history=${after.history_count};expected=${expectedCount}`);
    }
    if (!rollback && Number(after.template_count) !== 1) throw new Error(`QA_CERT_TEMPLATE_POSTCONDITION:${after.template_count}`);
    report.changed = Number(before.type_count) !== Number(after.type_count) || Number(before.history_count) !== Number(after.history_count);
    report.success = true;
    if (reportPath) writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
    console.log(JSON.stringify({ success: true, mode, changed: report.changed, after }));
  } catch (error) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch((error) => {
    console.error(`STAGING_QA_CERT_CANDIDATE_FAILED: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  });
}
