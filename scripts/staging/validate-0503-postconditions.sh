...
# operational_decision: staging-only read validation for Conhecimento Ativo 0503
...
[[ "$target" == "$ALLOWED_DB_NAME" && "$target" != "$BLOCKED_PRODUCTION_DB_NAME" ]] || {
  echo "ERROR: staging 0503 validator refused target: $target" >&2
  exit 1
}
...
echo CONHECIMENTO_ATIVO_FOUNDATION_0503_STAGING_POSTCONDITIONS=PASS
