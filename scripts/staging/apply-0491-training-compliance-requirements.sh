#!/usr/bin/env bash
# Governed staging-only application path for Schema V2 training compliance 0491.
...
SCHEMA_CHANGE_ID="training-compliance-requirements-0491"
apply=false
migration_arg=""
for arg in "$@"; do
  case "$arg" in
...
[[ "$migration_arg" == "$expected_path" ]] || { echo "ERROR: 0491 requires exact path $expected_path" >&2; exit 1; }
[[ ! -L "$migration_arg" && -f "$migration_arg" ]] || { echo "ERROR: migration 0491 missing or symlink refused" >&2; exit 1; }
if ! git -C release diff --quiet -- "worker-airtrust/migrations/$MIGRATION_BASENAME" || ! git -C release diff --cached --quiet -- "worker-airtrust/migrations/$MIGRATION_BASENAME"; then
  echo "ERROR: migration 0491 has uncommitted release changes" >&2; exit 1
fi
...
plan_path="release/worker-airtrust/schema-v2/plans/training-compliance-requirements-0491.md"
...
if(m.changeId!=='training-compliance-requirements-0491') throw new Error('CHANGE_ID_MISMATCH');
if(m.baselineId!=='production-d1-baseline-v2-20260714') throw new Error('BASELINE_ID_MISMATCH');
if(m.filePath!=='worker-airtrust/schema-v2/changes/0491_training_compliance_requirements.sql') throw new Error('FILE_PATH_MISMATCH');
if(m.planPath!=='worker-airtrust/schema-v2/plans/training-compliance-requirements-0491.md') throw new Error('PLAN_PATH_MISMATCH');
...
for table in matriz_treinamento_funcao qualificacoes_tipos setores funcoes funcionarios; do [[ "$(query_count "SELECT COUNT(*) AS count FROM sqlite_master WHERE type='table' AND name='$table';")" == 1 ]] || { echo "ERROR: prerequisite $table missing" >&2; exit 1; }; done
ledger_count="$(query_count "SELECT COUNT(*) AS count FROM d1_migrations WHERE name='$MIGRATION_BASENAME';")"
table_count="$(query_count "SELECT COUNT(*) AS count FROM sqlite_master WHERE type='table' AND name='treinamento_requisitos';")"
if [[ "$ledger_count" == 1 ]]; then
  [[ "$table_count" == 1 ]] || { echo "ERROR: 0491 ledger exists without target table" >&2; exit 1; }
  bash scripts/staging/validate-0491-postconditions.sh --target="$db_name"
  echo "MIGRATION_ALREADY_APPLIED_AND_VALIDATED=$MIGRATION_BASENAME"; exit 0
fi
[[ "$ledger_count" == 0 ]] || { echo "ERROR: invalid 0491 ledger count=$ledger_count" >&2; exit 1; }
[[ "$table_count" == 0 ]] || { echo "ERROR: 0491 schema drift: table exists without ledger" >&2; exit 1; }
preflight="$(mktemp -t airtrust-staging-0491-preflight.XXXXXXXX)"; recovery="$(mktemp -t airtrust-staging-0491-recovery.XXXXXXXX)"; combined="$(mktemp -t airtrust-staging-0491.XXXXXXXX.sql)"; trap 'rm -f "$preflight" "$recovery" "$combined"' EXIT
node scripts/staging/migration-ledger-preflight.mjs --scope="0491" > "$preflight" || { cat "$preflight" >&2; exit 1; }
...
[[ "$(query_count "SELECT COUNT(*) AS count FROM d1_migrations WHERE name='$MIGRATION_BASENAME';")" == 1 ]] || { echo "ERROR: 0491 applied without exact ledger row" >&2; exit 1; }
bash scripts/staging/validate-0491-postconditions.sh --target="$db_name"
