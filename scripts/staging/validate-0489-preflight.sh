#!/usr/bin/env bash
# Read-only fail-closed staging preflight for A-02 migration 0489.
set -euo pipefail
umask 077
...
[[ "$target" == "$ALLOWED_DB_NAME" ]] || {
  echo "ERROR: 0489 staging preflight refused target: $target" >&2
  exit 1
}
...
assert_count "tenant-index-contract:qualificacoes_tipos.codigo" "1" "SELECT COUNT(*) AS count FROM sqlite_master WHERE type = 'index' AND name = 'idx_qualificacoes_tipos_codigo_empresa_active' AND LOWER(REPLACE(REPLACE(REPLACE(REPLACE(COALESCE(sql,''),' ',''),char(9),''),char(10),''),char(13),'')) = 'createuniqueindexidx_qualificacoes_tipos_codigo_empresa_activeonqualificacoes_tipos(empresa_id,codigocollatenocase)wheredeleted_atisnull';"
...
echo "A02_NATURAL_KEYS_0489_STAGING_PREFLIGHT=PASS"
