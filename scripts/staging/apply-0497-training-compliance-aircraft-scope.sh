...
CONFIRMATION_PHRASE="AIRTRUST_STAGING_SCHEMA_CHANGE"; MIGRATION_BASENAME="0497_training_compliance_aircraft_scope.sql"; SCHEMA_CHANGE_ID="training-compliance-aircraft-scope-0497"
...
expected_path="release/worker-airtrust/migrations/$MIGRATION_BASENAME"; [[ "$migration_arg" == "$expected_path" ]] || { echo "ERROR: 0497 requires exact path $expected_path" >&2; exit 1; }
[[ ! -L "$migration_arg" && -f "$migration_arg" ]] || { echo "ERROR: migration 0497 missing or symlink refused" >&2; exit 1; }
if ! git -C release diff --quiet -- "worker-airtrust/migrations/$MIGRATION_BASENAME" || ! git -C release diff --cached --quiet -- "worker-airtrust/migrations/$MIGRATION_BASENAME"; then echo "ERROR: migration 0497 has uncommitted release changes" >&2; exit 1; fi
...
manifest_path="release/worker-airtrust/schema-v2/$SCHEMA_CHANGE_ID.json"; schema_sql_path="release/worker-airtrust/schema-v2/changes/$MIGRATION_BASENAME"; plan_path="release/worker-airtrust/schema-v2/plans/training-compliance-aircraft-scope-0497.md"
...
const fs=require('node:fs');const[,,p,sqlHash,planHash]=process.argv,m=JSON.parse(fs.readFileSync(p,'utf8'));if(m.changeId!=='training-compliance-aircraft-scope-0497'||m.baselineId!=='production-d1-baseline-v2-20260714'||m.filePath!=='worker-airtrust/schema-v2/changes/0497_training_compliance_aircraft_scope.sql'||m.planPath!=='worker-airtrust/schema-v2/plans/training-compliance-aircraft-scope-0497.md'||m.fileHash!==sqlHash||m.planHash!==planHash)throw new Error('REVIEWED_MANIFEST_MISMATCH');
NODE
...
for table in treinamento_requisitos funcionarios_aeronaves aeronaves; do [[ "$(query_count "SELECT COUNT(*) count FROM sqlite_master WHERE type='table' AND name='$table';")" == 1 ]] || { echo "ERROR: prerequisite $table missing" >&2; exit 1; }; done
[[ "$(query_count "SELECT COUNT(*) count FROM d1_migrations WHERE name='0491_training_compliance_requirements.sql';")" == 1 ]] || { echo "ERROR: prerequisite 0491 not applied in staging ledger" >&2; exit 1; }
ledger_count="$(query_count "SELECT COUNT(*) count FROM d1_migrations WHERE name='$MIGRATION_BASENAME';")"; column_count="$(query_count "SELECT COUNT(*) count FROM pragma_table_info('treinamento_requisitos') WHERE name='aeronave_modelo';")"
if [[ "$ledger_count" == 1 ]]; then [[ "$column_count" == 1 ]] || { echo "ERROR: 0497 ledger exists without target column" >&2; exit 1; }; bash scripts/staging/validate-0497-postconditions.sh --target="$db_name"; echo "MIGRATION_ALREADY_APPLIED_AND_VALIDATED=$MIGRATION_BASENAME"; exit 0; fi
[[ "$ledger_count" == 0 && "$column_count" == 0 ]] || { echo "ERROR: 0497 schema/ledger drift" >&2; exit 1; }
...
[[ "$(query_count "SELECT COUNT(*) count FROM d1_migrations WHERE name='$MIGRATION_BASENAME';")" == 1 ]] || { echo "ERROR: 0497 applied without exact ledger row" >&2; exit 1; }
bash scripts/staging/validate-0497-postconditions.sh --target="$db_name"; echo "MIGRATION_APPLIED_AND_VALIDATED=$MIGRATION_BASENAME"
