...
ALLOWED_DB_NAME="airtrust-db"; BASELINE_ID="production-d1-baseline-v2-20260714"; CHANGE_ID="controle-voos-petrobras-rve-export-0510"
target="$ALLOWED_DB_NAME"
for arg in "$@"; do case "$arg" in --target=*) target="${arg#*=}" ;; *) echo "ERROR: unknown argument: $arg" >&2; exit 1 ;; esac; done
[[ "$target" == "$ALLOWED_DB_NAME" ]] || { echo "ERROR: 0510 production preflight refused target: $target" >&2; exit 1; }
...
assert_count flights-table 1 "SELECT COUNT(*) count FROM sqlite_master WHERE type='table' AND name='cv_voos';"
assert_count source-columns 2 "SELECT COUNT(*) count FROM pragma_table_info('cv_voos') WHERE name IN ('sigvoos_flight_report_id','sigvoos_flight_report_id_confident');"
assert_count rve-columns-absent 0 "SELECT COUNT(*) count FROM pragma_table_info('cv_voos') WHERE name IN ('petrobras_equipamento','petrobras_atendimento');"
echo CONTROLE_VOOS_PETROBRAS_RVE_EXPORT_0510_PRODUCTION_PREFLIGHT=PASS
