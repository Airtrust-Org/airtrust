...
[[ "$target" == "$ALLOWED_DB_NAME" ]] || { echo "ERROR: 0504 production preflight refused target: $target" >&2; exit 1; }
...
assert_count new-tables-absent 0 "SELECT COUNT(*) count FROM sqlite_master WHERE type='table' AND name IN ('cv_contratos','cv_funcoes_bordo');"
echo CONTROLE_VOOS_OPERATIONAL_MODEL_0504_PRODUCTION_PREFLIGHT=PASS
