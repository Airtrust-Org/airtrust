...
[[ "$target" == "$ALLOWED_DB_NAME" ]] || { echo "ERROR: 0501 production preflight refused target: $target" >&2; exit 1; }
...
assert_count etapas-table 1 "SELECT COUNT(*) count FROM sqlite_master WHERE type='table' AND name='cv_voo_etapas';"
assert_count aeronaves-new-columns-absent 0 "SELECT COUNT(*) count FROM pragma_table_info('aeronaves') WHERE name IN ('peso_vazio','unidade_peso');"
assert_count etapas-new-columns-absent 0 "SELECT COUNT(*) count FROM pragma_table_info('cv_voo_etapas') WHERE name IN ('peso_passageiros','peso_bagagem','peso_tripulacao','peso_vazio','peso_total','unidade_peso','observacoes');"
...
echo CONTROLE_VOOS_LEG_OPERATIONAL_WEIGHTS_0501_PRODUCTION_PREFLIGHT=PASS
