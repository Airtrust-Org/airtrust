...
PREREQUISITE_CHANGE_ID="controle-voos-flight-justifications-0506"
target="$ALLOWED_DB_NAME"
for arg in "$@"; do case "$arg" in --target=*) target="${arg#*=}" ;; *) echo "ERROR: unknown argument: $arg" >&2; exit 1 ;; esac; done
[[ "$target" == "$ALLOWED_DB_NAME" ]] || { echo "ERROR: 0507 production preflight refused target: $target" >&2; exit 1; }
query_count() {
  local sql="$1"
...
assert_count prerequisite-0506-applied 1 "SELECT COUNT(*) count FROM airtrust_schema_changes_v2 WHERE change_id='$PREREQUISITE_CHANGE_ID';"
assert_count unapplied-change 0 "SELECT COUNT(*) count FROM airtrust_schema_changes_v2 WHERE change_id='$CHANGE_ID';"
assert_count category-column-absent 0 "SELECT COUNT(*) count FROM pragma_table_info('cv_justificativas_voo') WHERE name='categoria';"
assert_count tenant-6-present 1 "SELECT COUNT(*) count FROM empresas WHERE id=6;"
assert_count tenant-6-catalog-empty 0 "SELECT COUNT(*) count FROM cv_justificativas_voo WHERE empresa_id=6 AND deleted_at IS NULL;"
echo CONTROLE_VOOS_DELAY_JUSTIFICATION_CATALOG_0507_PRODUCTION_PREFLIGHT=PASS
