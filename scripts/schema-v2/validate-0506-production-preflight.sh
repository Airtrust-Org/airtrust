...
PREREQUISITE_CHANGE_ID="controle-voos-operational-model-0504"
...
[[ "$target" == "$ALLOWED_DB_NAME" ]] || { echo "ERROR: 0506 production preflight refused target: $target" >&2; exit 1; }
query_count() {
  local sql="$1"
...
assert_count prerequisite-0504-applied 1 "SELECT COUNT(*) count FROM airtrust_schema_changes_v2 WHERE change_id='$PREREQUISITE_CHANGE_ID';"
assert_count unapplied-change 0 "SELECT COUNT(*) count FROM airtrust_schema_changes_v2 WHERE change_id='$CHANGE_ID';"
assert_count catalog-table-absent 0 "SELECT COUNT(*) count FROM sqlite_master WHERE type='table' AND name='cv_justificativas_voo';"
assert_count link-table-absent 0 "SELECT COUNT(*) count FROM sqlite_master WHERE type='table' AND name='cv_voo_justificativas';"
echo CONTROLE_VOOS_FLIGHT_JUSTIFICATIONS_0506_PRODUCTION_PREFLIGHT=PASS
