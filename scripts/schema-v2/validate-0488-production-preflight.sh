#!/usr/bin/env bash
# Read-only fail-closed production preflight for reviewed Schema V2 change 0488.
set -euo pipefail
umask 077
...
if [[ "$target" != "$ALLOWED_DB_NAME" ]]; then
  echo "ERROR: 0488 production preflight refused target: $target" >&2
  exit 1
fi
...
for prerequisite in cv_voos cv_rdv_operacional cv_voo_etapas usuarios; do
  ...
  [[ "$count" == "1" ]] || { echo "ERROR: prerequisite table $prerequisite is absent" >&2; exit 1; }
done

table_count="$(query_count "SELECT COUNT(*) AS count FROM sqlite_master WHERE type = 'table' AND name = 'cv_offline_sync_receipts';")"
[[ "$table_count" == "0" ]] || {
  echo "ERROR: CV_OFFLINE_SYNC_RECEIPTS_SCHEMA_DRIFT: table already exists without reviewed 0488 ledger row" >&2
  exit 1
}

echo "CONTROLE_VOOS_PILOT_OFFLINE_SYNC_RECEIPTS_0488_PRODUCTION_PREFLIGHT=PASS"
