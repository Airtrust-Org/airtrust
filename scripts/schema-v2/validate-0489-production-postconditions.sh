#!/usr/bin/env bash
# Read-only production postconditions for A-02 Schema V2 change 0489.
set -euo pipefail
umask 077
...
[[ "$target" == "$ALLOWED_DB_NAME" ]] || {
  echo "ERROR: 0489 production postconditions refused target: $target" >&2
  exit 1
}
...
echo "A02_NATURAL_KEYS_0489_PRODUCTION_POSTCONDITIONS=PASS"
