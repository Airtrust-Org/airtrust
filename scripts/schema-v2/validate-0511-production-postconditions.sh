...
ALLOWED_DB_NAME="airtrust-db"; BASELINE_ID="production-d1-baseline-v2-20260714"; CHANGE_ID="frms-duty-boundary-config-0511"
target="$ALLOWED_DB_NAME"
for arg in "$@"; do case "$arg" in --target=*) target="${arg#*=}" ;; *) echo "ERROR: unknown argument: $arg" >&2; exit 1 ;; esac; done
[[ "$target" == "$ALLOWED_DB_NAME" ]] || { echo "ERROR: 0511 production postconditions refused target: $target" >&2; exit 1; }
...
echo FRMS_DUTY_BOUNDARY_CONFIG_0511_PRODUCTION_POSTCONDITIONS=PASS
