...
ALLOWED_DB_NAME="airtrust-db"; BASELINE_ID="production-d1-baseline-v2-20260714"; CHANGE_ID="controle-voos-flight-plan-0508"
target="$ALLOWED_DB_NAME"
for arg in "$@"; do case "$arg" in --target=*) target="${arg#*=}" ;; *) echo "ERROR: unknown argument: $arg" >&2; exit 1 ;; esac; done
[[ "$target" == "$ALLOWED_DB_NAME" ]] || { echo "ERROR: 0508 production postconditions refused target: $target" >&2; exit 1; }
...
assert_count flight-plan-tables 2 "SELECT COUNT(*) count FROM sqlite_master WHERE type='table' AND name IN ('cv_planos_voo','cv_plano_voo_eventos');"
...
assert_count flight-plan-triggers 5 "SELECT COUNT(*) count FROM sqlite_master WHERE type='trigger' AND name IN ('trg_cv_planos_voo_tenant_insert_0508','trg_cv_planos_voo_link_immutable_0508','trg_cv_plano_voo_eventos_tenant_insert_0508','trg_cv_plano_voo_eventos_no_update_0508','trg_cv_plano_voo_eventos_no_delete_0508');"
...
echo CONTROLE_VOOS_FLIGHT_PLAN_0508_PRODUCTION_POSTCONDITIONS=PASS
