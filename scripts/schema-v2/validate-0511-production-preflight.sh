...
[[ "$target" == "$ALLOWED_DB_NAME" ]] || { echo "ERROR: 0511 production preflight refused target: $target" >&2; exit 1; }
...
assert_count duty-config-columns-absent 0 "SELECT COUNT(*) count FROM pragma_table_info('frms_fadiga_config_empresa') WHERE name IN ('jornada_pos_corte_minutos','jornada_sem_voo_fim');"
echo FRMS_DUTY_BOUNDARY_CONFIG_0511_PRODUCTION_PREFLIGHT=PASS
