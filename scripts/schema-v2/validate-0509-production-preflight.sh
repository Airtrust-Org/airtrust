...
ALLOWED_DB_NAME="airtrust-db"; BASELINE_ID="production-d1-baseline-v2-20260714"; CHANGE_ID="controle-voos-flight-plan-0509"
target="$ALLOWED_DB_NAME"
for arg in "$@"; do case "$arg" in --target=*) target="${arg#*=}" ;; *) echo "ERROR: unknown argument: $arg" >&2; exit 1 ;; esac; done
[[ "$target" == "$ALLOWED_DB_NAME" ]] || { echo "ERROR: 0509 production preflight refused target: $target" >&2; exit 1; }
...
assert_count flight-plan-table-absent 0 "SELECT COUNT(*) count FROM sqlite_master WHERE type='table' AND name='cv_planos_voo';"
assert_count flight-plan-events-table-absent 0 "SELECT COUNT(*) count FROM sqlite_master WHERE type='table' AND name='cv_plano_voo_eventos';"
echo CONTROLE_VOOS_FLIGHT_PLAN_0509_PRODUCTION_PREFLIGHT=PASS
