...
ALLOWED_DB_NAME="airtrust-db"; BASELINE_ID="production-d1-baseline-v2-20260714"; CHANGE_ID="controle-voos-fueling-companies-0502"
target="$ALLOWED_DB_NAME"
for arg in "$@"; do case "$arg" in --target=*) target="${arg#*=}" ;; *) echo "ERROR: unknown argument: $arg" >&2; exit 1 ;; esac; done
[[ "$target" == "$ALLOWED_DB_NAME" ]] || { echo "ERROR: 0502 production preflight refused target: $target" >&2; exit 1; }
...
assert_count historical-fueling-table 1 "SELECT COUNT(*) count FROM sqlite_master WHERE type='table' AND name='cv_voo_abastecimentos';"
assert_count fueling-company-table-absent 0 "SELECT COUNT(*) count FROM sqlite_master WHERE type='table' AND name='cv_empresas_abastecimento';"
echo CONTROLE_VOOS_FUELING_COMPANIES_0502_PRODUCTION_PREFLIGHT=PASS
