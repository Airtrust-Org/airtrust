...
READINESS_AUDIT="$ROOT_DIR/scripts/audit-migration-chain-readiness.sh"
...
[[ -f "$READINESS_DOC" ]] || add_reason "mig01_readiness_doc_missing"
...
if (( ${#fail_reasons[@]} > 0 )); then
  echo "MIG01_REBASELINE_GATE=BLOCKED_BY_ENVIRONMENT_READINESS"
...
db_path="${AIRTRUST_DB_PATH:-${AIRTRUST_MIG01_DB_PATH:-}}"
...
snapshot_ref="${AIRTRUST_CONTROLLED_SNAPSHOT_REF:-${AIRTRUST_MIG01_SNAPSHOT_REF:-}}"
rollback_path="${AIRTRUST_CONTROLLED_ROLLBACK_PATH:-${AIRTRUST_MIG01_ROLLBACK_PLAN_PATH:-}}"
rollback_ref="${AIRTRUST_CONTROLLED_ROLLBACK_REF:-${AIRTRUST_MIG01_ROLLBACK_REF:-}}"
safe_command="${AIRTRUST_CONTROLLED_SAFE_COMMAND:-${AIRTRUST_MIG01_SAFE_COMMAND:-}}"
safe_command_reviewed="${AIRTRUST_CONTROLLED_SAFE_COMMAND_REVIEWED:-${AIRTRUST_MIG01_SAFE_COMMAND_REVIEWED:-}}"
allow_production="${AIRTRUST_CONTROLLED_PRODUCTION_APPROVED:-${AIRTRUST_MIG01_PRODUCTION_APPROVED:-}}"
allow_remote_d1="${AIRTRUST_CONTROLLED_ALLOW_REMOTE_D1:-${AIRTRUST_MIG01_ALLOW_REMOTE_D1:-NO}}"
...
if output="$(
  ...
  echo "MIG01_REBASELINE_GATE=READY_FOR_MANUAL_CONTROLLED_EXECUTION"
  printf '%s\n' "$output"
  echo "NOTE=MIG01 gate passed. No rebaseline was executed by this script."
else
  status=$?
  echo "MIG01_REBASELINE_GATE=BLOCKED_BY_ENVIRONMENT_READINESS"
  printf '%s\n' "$output"
