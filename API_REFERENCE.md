...
**Base URL:** `https://api.airtrust.online/api`

> **Nota:** A versão atual não usa prefixo `/v2`. Os endpoints listados abaixo
> refletem a estrutura vigente. O documento está em revisão.
...
```bash
curl -H "Authorization: Bearer eyJhbGc..." https://api.airtrust.online/api/funcionarios
```
...
```bash
curl -X GET \
  'https://api.airtrust.online/api/funcionarios?page=1&limit=10' \
...
```bash
curl -X POST \
  https://api.airtrust.online/api/funcionarios \
  -H 'Authorization: Bearer token-aqui' \
...
```bash
curl -X POST \
  https://api.airtrust.online/api/agendamentos \
  -H 'Authorization: Bearer token-aqui' \
  -H 'Content-Type: application/json' \
  -d '{
    "funcionario_id": "uuid",
